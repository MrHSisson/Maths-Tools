...
**Ratio table** (`src/shared/ratioTable.ts` + `src/shared/components/RatioTable.tsx`) is a *working-step* representation rather than a Teach-deck scene, rendered as a genuine bordered `<table>` — matching how a ratio table is conventionally drawn on paper: **one row per quantity** (the row label in the left-hand cell, e.g. "Miles", "Hours"), **one column per scale-step**, with the scale factor shown in its own cell (spanning every quantity row) between adjacent step-columns (e.g. Miles row `30 → 15`, Hours row `2 → 1`, with `÷2` spanning both rows in between). Author one via `rStep(label, headers, rows, operations)` (alongside `step`/`mStep`/`tStep`) — `headers` are the row labels, `rows` is one array of values per step (so `rows[step][quantityIndex]`, NOT one array per quantity), `operations` is the factor between each consecutive step. Pass the shared `ratioTableStepRenderer` as the tool's `stepRenderer` — it returns `null` for every non-ratio-table step, so `mStep`/`tStep`/`step` still render through ToolShell's normal path. A multi-step scale is just more entries in `rows`/`operations` in one call, or several `rStep` working steps in sequence (one per press in Step-by-Step mode) — there is no fragment-level reveal inside a single ratio table. Reference implementation: `src/tools/Proportion/SpeedDistanceTime.tsx`.
