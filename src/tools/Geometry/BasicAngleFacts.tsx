...
const mkNumType = (): MultiSelectCfg => ({ key: "numberType", label: "Number Type", options: [{ value: "integer", label: "Integers", defaultActive: true }, { value: "decimal", label: "Decimals", defaultActive: false }] });
const mkParts = (): MultiSelectCfg => ({ key: "parts", label: "Parts", options: [{ value: "two", label: "2 parts", defaultActive: true }, { value: "three", label: "3 parts", defaultActive: true }] });
const mkAlgL12 = (): MultiSelectCfg => ({ key: "algOptions", label: "Algebraic Options", options: [{ value: "coefficient", label: "Coefficient|(e.g. 2x)", defaultActive: false }, { value: "constant", label: "Constant|(e.g. x+14)", defaultActive: false }, { value: "both", label: "Both|(e.g. 2x+30)", defaultActive: false }] });
const mkAlgL3 = (): MultiSelectCfg => ({ key: "algOptions", label: "Algebraic Options", options: [{ value: "useCoefficients", label: "Coefficients|(e.g. 2x)", defaultActive: false }] });
const mkVOL1Variants = (): MultiSelectCfg => ({ key: "voL1Variants", label: "Question Type", options: [{ value: "matching", label: "Matching|(identify x & y)", defaultActive: true }, { value: "calculation", label: "Calculation|(find x & y)", defaultActive: true }] });
const mkVOL2Variants = (): MultiSelectCfg => ({ key: "voL2Variants", label: "Question Type", options: [{ value: "findX", label: "Find x|(both subs given)", defaultActive: true }, { value: "findSub", label: "Find sub|(x + sub = VO)", defaultActive: true }] });
const mkVOL2Alg = (): MultiSelectCfg => ({ key: "algOptions", label: "Algebraic Options", options: [{ value: "coefficient", label: "Coefficient|(e.g. 2x)", defaultActive: false }, { value: "constant", label: "Constant|(e.g. x+14)", defaultActive: false }, { value: "both", label: "Both|(e.g. 2x+30)", defaultActive: false }] });
const mkVOL3Variants = (): MultiSelectCfg => ({ key: "voL3Variants", label: "Question Type", options: [{ value: "pureVO", label: "Pure VO|(two expressions)", defaultActive: true }, { value: "subdivided", label: "Subdivided|(sum = expression)", defaultActive: false }] });

const TOOL_CONFIG: { pageTitle: string; tools: Record<ToolKey, ToolCfg> } = {
  pageTitle: "Basic Angle Facts",
  tools: {
    rightAngle: { name: "Right Angle", difficultySettings: { level1: { variables: [{ key: "showSquare", label: "Show right angle square symbol", defaultValue: true }], multiSelect: mkAlgL12(), multiSelect2: mkNumType() }, level2: { variables: [{ key: "showSquare", label: "Show right angle square symbol", defaultValue: true }], multiSelect: mkAlgL12(), multiSelect2: mkNumType() }, level3: { variables: [{ key: "showSquare", label: "Show right angle square symbol", defaultValue: true }], multiSelect: mkAlgL3(), multiSelect3: mkParts() } } },
    straightLine: { name: "Straight Line", difficultySettings: { level1: { variables: [{ key: "fixedRotation", label: "Always horizontal (no rotation)", defaultValue: false }], multiSelect: mkAlgL12(), multiSelect2: mkNumType() }, level2: { variables: [{ key: "fixedRotation", label: "Always horizontal (no rotation)", defaultValue: false }], multiSelect: mkAlgL12(), multiSelect2: mkNumType() }, level3: { variables: [{ key: "fixedRotation", label: "Always horizontal (no rotation)", defaultValue: false }], multiSelect: mkAlgL3(), multiSelect3: mkParts() } } },
    aroundPoint: { name: "Around a Point", difficultySettings: { level1: { variables: [], multiSelect: mkAlgL12(), multiSelect2: mkNumType() }, level2: { variables: [], multiSelect: mkAlgL12(), multiSelect2: mkNumType() }, level3: { variables: [], multiSelect: mkAlgL3(), multiSelect3: mkParts() } } },
    verticallyOpposite: { name: "Vertically Opposite", difficultySettings: { level1: { variables: [], multiSelect: mkVOL1Variants() }, level2: { variables: [], multiSelect: mkVOL2Variants(), multiSelect2: mkVOL2Alg() }, level3: { variables: [], multiSelect: mkVOL3Variants(), multiSelect2: mkAlgL3() } } },
    mixed: { name: "Mixed Practice", difficultySettings: { level1: { variables: [{ key: "showSquare", label: "Show right angle square symbol", defaultValue: true }], multiSelect: mkAlgL12(), multiSelect2: mkNumType(), multiSelect3: { key: "angleType", label: "Angle Types", options: [{ value: "rightAngle", label: "90°", defaultActive: true }, { value: "straightLine", label: "180°", defaultActive: true }, { value: "aroundPoint", label: "360°", defaultActive: true }, { value: "verticallyOpposite", label: "Vert. Opp.", defaultActive: true }] } }, level2: { variables: [{ key: "showSquare", label: "Show right angle square symbol", defaultValue: true }], multiSelect: mkAlgL12(), multiSelect2: mkNumType(), multiSelect3: { key: "angleType", label: "Angle Types", options: [{ value: "rightAngle", label: "90°", defaultActive: true }, { value: "straightLine", label: "180°", defaultActive: true }, { value: "aroundPoint", label: "360°", defaultActive: true }, { value: "verticallyOpposite", label: "Vert. Opp.", defaultActive: true }] } }, level3: { variables: [], multiSelect: mkAlgL3(), multiSelect3: { key: "angleType", label: "Angle Types", options: [{ value: "rightAngle", label: "90°", defaultActive: true }, { value: "straightLine", label: "180°", defaultActive: true }, { value: "aroundPoint", label: "360°", defaultActive: true }, { value: "verticallyOpposite", label: "Vert. Opp.", defaultActive: true }] } } } },
  },
};
...
const LV_COLORS: Record<string, { bg: string; border: string; text: string; fill: string }> = { level1: { bg: "bg-green-50", border: "border-green-500", text: "text-green-700", fill: "#dcfce7" }, level2: { bg: "bg-yellow-50", border: "border-yellow-500", text: "text-yellow-700", fill: "#fef9c3" }, level3: { bg: "bg-red-50", border: "border-red-500", text: "text-red-700", fill: "#fee2e2" } };
const INFO_SECTIONS = [
...
function getQuestionBg(cs: string): string { const m: Record<string, string> = { blue: "#D1E7F8", pink: "#F8D1E7", yellow: "#F8F4D1" }; return m[cs] ?? "#ffffff"; }
function getStepBg(cs: string): string { const m: Record<string, string> = { blue: "#B3D9F2", pink: "#F2B3D9", yellow: "#F2EBB3" }; return m[cs] ?? "#f3f4f6"; }
...
function buildVarsFromUI(tool: string, level: string, toolVariables: Record<string, Record<string, Record<string, boolean>>>, toolMultiSelect: Record<string, Record<string, boolean>>): QOVars {
  const t = TOOL_CONFIG.tools[tool as ToolKey]; const ds = t.difficultySettings[level as DifficultyLevel];
  const vars: QOVars = { ...(toolVariables[tool]?.[level] ?? {}) };
  const applyMS = (ms: MultiSelectCfg | undefined, prefix = "") => { if (!ms) return; ms.options.forEach(o => { const stateKey = prefix ? `${level}__${prefix}__${o.value}` : `${level}__${o.value}`; vars[o.value] = toolMultiSelect[tool]?.[stateKey] ?? o.defaultActive; }); };
...
function getUniqueQuestion(tool: string, level: string, vars: QOVars, used: Set<string>): AngleQuestion {
  ...
  used.add(q.key ?? ""); q._qo = JSON.parse(JSON.stringify(vars)); q._tool = tool; return q;
}
...
function handlePrint(worksheet: AngleQuestion[], isDiff: boolean, toolName: string, ref: React.RefObject<HTMLDivElement>) {
  ...
  const cell = (q: AngleQuestion, gi: number, li: number, showAns: boolean) => { const ansText = showAns ? (q.answer2 ? `${q.answer}, ${q.answer2}` : q.answer) : ""; return `<div class="cell"><div class="cell-num">${li + 1}</div><div class="cell-diagram">${svgStrings[gi] ?? ""}</div>${showAns ? `<div class="answer">${ansText}</div>` : ""}</div>`; };
  const stdPage = (qs: AngleQuestion[], start: number, pn: number, tp: number, ans: boolean) => { const cells = qs.map((q, i) => cell(q, start + i, start + i, ans)).join(""); return `<div class="page"><div class="ph"><h1>${toolName}${ans ? " \u2014 Answers" : ""}</h1><div class="meta">Worksheet &middot; ${dateStr}${tp > 1 ? ` &middot; Page ${pn} of ${tp}` : ""}</div></div><div class="sg">${cells}</div></div>`; };
  ...
  const buildDiff = (ans: boolean) => { const byLv: Record<string, AngleQuestion[]> = { level1: worksheet.filter(q => q.level === "level1"), level2: worksheet.filter(q => q.level === "level2"), level3: worksheet.filter(q => q.level === "level3") }; const off: Record<string, number> = { level1: 0, level2: byLv.level1.length, level3: byLv.level1.length + byLv.level2.length }; const tp = Math.ceil(Math.max(...Object.values(byLv).map(a => a.length)) / PRINT_ROWS); return Array.from({ length: tp }, (_, p) => { const cols = ["level1", "level2", "level3"].map((lv, li) => { const nc = ["#166534", "#854d0e", "#991b1b"][li], bg = ["#dcfce7", "#fef9c3", "#fee2e2"][li], lb = ["Level 1", "Level 2", "Level 3"][li]; const qs = byLv[lv].slice(p * PRINT_ROWS, (p + 1) * PRINT_ROWS); const cells = qs.map((q, i) => cell(q, off[lv] + p * PRINT_ROWS + i, p * PRINT_ROWS + i, ans)).join(""); return `<div class="dc"><div class="dh" style="color:${nc};background:${bg}">${lb}</div><div class="dcs">${cells}</div></div>`; }).join(""); return `<div class="page"><div class="ph"><h1>${toolName}${ans ? " \u2014 Answers" : ""}</h1><div class="meta">Differentiated &middot; ${dateStr}${tp > 1 ? ` &middot; Page ${p + 1} of ${tp}` : ""}</div></div><div class="dg">${cols}</div></div>`; }).join(""); };
  ...
  const win = window.open("", "_blank"); if (!win) { alert("Please allow popups."); return; } win.document.write(html); win.document.close(); setTimeout(() => win.print(), 400);
}

// ─── SHARED UI ────────────────────────────────────────────────────────────────
const usePopover = () => { const [open, setOpen] = useState(false); const ref = useRef<HTMLDivElement>(null); useEffect(() => { if (!open) return; const h = (e: MouseEvent) => { if (ref.current && !ref.current.contains(e.target as Node)) setOpen(false); }; document.addEventListener("mousedown", h); return () => document.removeEventListener("mousedown", h); }, [open]); return { open, setOpen, ref }; };
const Chevron = ({ open }: { open: boolean }) => (<svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ transition: "transform 0.2s", transform: open ? "rotate(180deg)" : "rotate(0deg)", flexShrink: 0 }}><path d="M4 6l4 4 4-4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>);
const PopoverButton = ({ open, onClick }: { open: boolean; onClick: () => void }) => (<button onClick={onClick} className={`px-4 py-2 rounded-xl border-2 font-bold text-base transition-colors shadow-sm flex items-center gap-2 ${open ? "bg-blue-900 border-blue-900 text-white" : "bg-white border-gray-300 text-gray-600 hover:border-blue-900 hover:text-blue-900"}`}>Question Options <Chevron open={open} /></button>);
const DiffToggle = ({ value, onChange }: { value: string; onChange: (v: string) => void }) => (<div className="flex rounded-xl border-2 border-gray-300 overflow-hidden shadow-sm">{[["level1", "Level 1", "bg-green-600"], ["level2", "Level 2", "bg-yellow-500"], ["level3", "Level 3", "bg-red-600"]].map(([val, lbl, col]) => (<button key={val} onClick={() => onChange(val)} className={`px-5 py-2 font-bold text-base transition-colors ${value === val ? `${col} text-white` : "bg-white text-gray-500 hover:bg-gray-50"}`}>{lbl}</button>))}</div>);
const VariablesSection = ({ variables, values, onChange }: { variables: VarCfg[]; values: Record<string, boolean>; onChange: (k: string, v: boolean) => void }) => (<div className="flex flex-col gap-3"><span className="text-sm font-bold text-gray-400 uppercase tracking-wider">Options</span>{variables.map(v => (<label key={v.key} className="flex items-center gap-3 cursor-pointer py-1"><div onClick={() => onChange(v.key, !values[v.key])} className={`w-12 h-6 rounded-full transition-colors relative flex-shrink-0 ${values[v.key] ? "bg-blue-900" : "bg-gray-300"}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${values[v.key] ? "translate-x-7" : "translate-x-1"}`} /></div><span className="text-base font-semibold text-gray-700">{v.label}</span></label>))}</div>);
const MultiSelectSection = ({ multiSelect, values, onChange, allowAllOff = false }: { multiSelect: MultiSelectCfg; values: Record<string, boolean>; onChange: (k: string, v: boolean) => void; allowAllOff?: boolean }) => { const ac = multiSelect.options.filter(o => values[o.value]).length; return (<div className="flex flex-col gap-2"><span className="text-sm font-bold text-gray-400 uppercase tracking-wider">{multiSelect.label}</span><div className="flex rounded-lg border-2 border-gray-200 overflow-hidden">{multiSelect.options.map(opt => { const isActive = values[opt.value] ?? false; const isLast = !allowAllOff && isActive && ac === 1; return (<button key={opt.value} onClick={() => { if (!isLast) onChange(opt.value, !isActive); }} className={`flex-1 px-3 py-2 text-sm font-bold transition-colors text-center ${isActive ? "bg-blue-900 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}>{opt.label.split("|").map((line, i) => <span key={i} style={{ display: "block" }}>{line}</span>)}</button>); })}</div></div>); };

function renderLevelPanelContent(tool: ToolKey, lv: DifficultyLevel, toolVariables: Record<string, Record<string, Record<string, boolean>>>, toolMultiSelect: Record<string, Record<string, boolean>>, setTV: (t: string, l: string, k: string, v: boolean) => void, setTMS: (t: string, l: string, k: string, v: boolean) => void) {
  const ds = TOOL_CONFIG.tools[tool].difficultySettings[lv]; const varVals = toolVariables[tool]?.[lv] ?? {};
  const msVals: Record<string, boolean> = {}; if (ds.multiSelect) ds.multiSelect.options.forEach(o => { msVals[o.value] = toolMultiSelect[tool]?.[`${lv}__${o.value}`] ?? o.defaultActive; });
  const ms2Vals: Record<string, boolean> = {}; if (ds.multiSelect2) ds.multiSelect2.options.forEach(o => { ms2Vals[o.value] = toolMultiSelect[tool]?.[`${lv}__ms2__${o.value}`] ?? o.defaultActive; });
  const ms3Vals: Record<string, boolean> = {}; if (ds.multiSelect3) ds.multiSelect3.options.forEach(o => { ms3Vals[o.value] = toolMultiSelect[tool]?.[`${lv}__ms3__${o.value}`] ?? o.defaultActive; });
  return (<div className="flex flex-col gap-4">{ds.multiSelect && <MultiSelectSection multiSelect={ds.multiSelect} values={msVals} onChange={(k, v) => setTMS(tool, lv, k, v)} allowAllOff={ds.multiSelect.key === "algOptions"} />}{ds.multiSelect2 && <MultiSelectSection multiSelect={ds.multiSelect2} values={ms2Vals} onChange={(k, v) => setTMS(tool, lv, `ms2__${k}`, v)} allowAllOff={ds.multiSelect2.key === "algOptions"} />}{ds.multiSelect3 && <MultiSelectSection multiSelect={ds.multiSelect3} values={ms3Vals} onChange={(k, v) => setTMS(tool, lv, `ms3__${k}`, v)} />}{ds.variables.length > 0 && <VariablesSection variables={ds.variables} values={varVals} onChange={(k, v) => setTV(tool, lv, k, v)} />}</div>);
}
...
const StandardQOPopover = ({ tool, level, toolVariables, setToolVariable, toolMultiSelect, setToolMultiSelect }: QOProps) => { const { open, setOpen, ref } = usePopover(); return (<div className="relative" ref={ref}><PopoverButton open={open} onClick={() => setOpen(!open)} />{open && <div className="absolute left-0 top-full mt-2 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 min-w-96 p-5">{renderLevelPanelContent(tool, level, toolVariables, toolMultiSelect, setToolVariable, setToolMultiSelect)}</div>}</div>); };
...
const DiffQOPopover = ({ tool, toolVariables, setToolVariable, toolMultiSelect, setToolMultiSelect }: DiffQOProps) => { const { open, setOpen, ref } = usePopover(); return (<div className="relative" ref={ref}><PopoverButton open={open} onClick={() => setOpen(!open)} />{open && (<div className="absolute left-0 top-full mt-2 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 min-w-96 p-5 flex flex-col gap-6">{(["level1", "level2", "level3"] as DifficultyLevel[]).map(lv => (<div key={lv}><span className={`text-sm font-extrabold uppercase tracking-wider ${LV_HEADER_COLORS[lv]}`}>{LV_LABELS[lv]}</span><div className="mt-3 pl-1">{renderLevelPanelContent(tool, lv, toolVariables, toolMultiSelect, setToolVariable, setToolMultiSelect)}</div></div>))}</div>)}</div>); };

const InfoModal = ({ onClose }: { onClose: () => void }) => (<div className="fixed inset-0 z-[100] flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.5)" }} onClick={onClose}><div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 flex flex-col" style={{ height: "80vh" }} onClick={e => e.stopPropagation()}><div className="flex items-center justify-between px-7 py-5 border-b border-gray-100 flex-shrink-0"><h2 className="text-2xl font-bold text-gray-900">Tool Information</h2><button onClick={onClose} className="w-9 h-9 rounded-full flex items-center justify-center text-gray-400 hover:bg-gray-100"><X size={20} /></button></div><div className="overflow-y-auto px-7 py-6 flex flex-col gap-6 flex-1">{INFO_SECTIONS.map(s => (<div key={s.title}><div className="flex items-center gap-2 mb-3"><span className="text-xl">{s.icon}</span><h3 className="text-lg font-bold text-blue-900">{s.title}</h3></div><div className="flex flex-col gap-2">{s.content.map(item => <div key={item.label} className="bg-gray-50 rounded-xl px-4 py-3"><span className="font-bold text-gray-800 text-sm">{item.label}</span><p className="text-sm text-gray-500 mt-0.5">{item.detail}</p></div>)}</div></div>))}</div><div className="px-7 py-4 border-t border-gray-100 flex justify-end flex-shrink-0"><button onClick={onClose} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-sm">Close</button></div></div></div>);
const MenuDropdown = ({ colorScheme, setColorScheme, onClose, onOpenInfo }: { colorScheme: string; setColorScheme: (s: string) => void; onClose: () => void; onOpenInfo: () => void }) => { const [colorOpen, setColorOpen] = useState(false); const ref = useRef<HTMLDivElement>(null); useEffect(() => { const h = (e: MouseEvent) => { if (ref.current && !ref.current.contains(e.target as Node)) onClose(); }; document.addEventListener("mousedown", h); return () => document.removeEventListener("mousedown", h); }, [onClose]); return (<div ref={ref} className="absolute right-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden" style={{ minWidth: 200 }}><div className="py-1"><button onClick={() => setColorOpen(!colorOpen)} className="w-full flex items-center justify-between px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50"><span>Colour Scheme</span><span className="text-xs text-gray-400 capitalize">{colorScheme}</span></button>{colorOpen && <div className="border-t border-gray-100">{["default", "blue", "pink", "yellow"].map(s => (<button key={s} onClick={() => { setColorScheme(s); onClose(); }} className={`w-full flex items-center pl-10 pr-4 py-2.5 text-sm font-semibold capitalize ${colorScheme === s ? "bg-blue-900 text-white" : "text-gray-600 hover:bg-gray-50"}`}>{s}</button>))}</div>}<div className="border-t border-gray-100 my-1" /><button onClick={() => { onOpenInfo(); onClose(); }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">Tool Information</button></div></div>); };
...
export default function BasicAngleFacts() {
  ...
  const initVars = (): Record<string, Record<string, Record<string, boolean>>> => { const r: Record<string, Record<string, Record<string, boolean>>> = {}; toolKeys.forEach(k => { r[k] = {}; (["level1","level2","level3"] as DifficultyLevel[]).forEach(lv => { r[k][lv] = {}; TOOL_CONFIG.tools[k].difficultySettings[lv].variables.forEach(v => { r[k][lv][v.key] = v.defaultValue; }); }); }); return r; };
  const initMS = (): Record<string, Record<string, boolean>> => { const r: Record<string, Record<string, boolean>> = {}; toolKeys.forEach(k => { r[k] = {}; (["level1","level2","level3"] as DifficultyLevel[]).forEach(lv => { const ds = TOOL_CONFIG.tools[k].difficultySettings[lv]; if (ds.multiSelect) ds.multiSelect.options.forEach(o => { r[k][`${lv}__${o.value}`] = o.defaultActive; }); if (ds.multiSelect2) ds.multiSelect2.options.forEach(o => { r[k][`${lv}__ms2__${o.value}`] = o.defaultActive; }); if (ds.multiSelect3) ds.multiSelect3.options.forEach(o => { r[k][`${lv}__ms3__${o.value}`] = o.defaultActive; }); }); }); return r; };
  ...
  const newQ = useCallback(() => { const v = buildVarsFromUI(currentTool, difficulty, toolVariables, toolMultiSelect); setCurQ(generateQuestion(currentTool, difficulty, v)); setShowWBA(false); setShowAns(false); }, [currentTool, difficulty, toolVariables, toolMultiSelect]);

  const genWorksheet = () => { const used = new Set<string>(); const qs: AngleQuestion[] = []; if (isDiff) { (["level1","level2","level3"] as DifficultyLevel[]).forEach(lv => { const vars = bvars(currentTool, lv); for (let i = 0; i < numQ; i++) qs.push(getUniqueQuestion(currentTool, lv, vars, used)); }); } else { const vars = bvars(currentTool, difficulty); for (let i = 0; i < numQ; i++) qs.push(getUniqueQuestion(currentTool, difficulty, vars, used)); } setWorksheet(qs); setShowWSAns(false); };

  const regenQ = (idx: number) => { const q = worksheet[idx]; if (!q._qo) return; const tool = (q._tool ?? currentTool) as ToolKey; const level = (q.difficulty ?? q.level) as DifficultyLevel; const vars = q._qo as QOVars; const existing = new Set(worksheet.map(w => w.key ?? "")); existing.delete(q.key ?? ""); let rep: AngleQuestion | null = null; for (let att = 0; att < 100; att++) { const c = generateQuestion(tool, level, vars); if (!existing.has(c.key ?? "")) { c._qo = JSON.parse(JSON.stringify(vars)); c._tool = tool; rep = c; break; } } if (rep) setWorksheet(prev => prev.map((w, i) => i === idx ? rep! : w)); };

  const genAdvanced = () => { const used = new Set<string>(); const qs: AngleQuestion[] = []; advGroups.forEach(g => { const ds = TOOL_CONFIG.tools[currentTool].difficultySettings[g.level]; const vars: QOVars = { ...g.variables, ...g.multiSelectValues, ...g.multiSelectValues2, ...g.multiSelectValues3 }; if (ds.multiSelect2) { vars.integer = g.multiSelectValues2.integer ?? true; vars.decimal = g.multiSelectValues2.decimal ?? false; } for (let i = 0; i < g.count; i++) qs.push(getUniqueQuestion(currentTool, g.level, vars, used)); }); if (advShuffle) { for (let i = qs.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [qs[i], qs[j]] = [qs[j], qs[i]]; } } setWorksheet(qs); setShowWSAns(false); };

  useEffect(() => { if (mode !== "worksheet") newQ(); }, [difficulty, currentTool]);
  useEffect(() => { if (mode !== "worksheet" || wsMode !== "advanced") return; const h = (e: KeyboardEvent) => { if (e.key !== "ArrowLeft" && e.key !== "ArrowRight") return; const idx = advGroups.findIndex(g => g.id === advSelId); if (idx < 0) return; const ni = e.key === "ArrowLeft" ? idx - 1 : idx + 1; if (ni >= 0 && ni < advGroups.length) { setAdvSelId(advGroups[ni].id); e.preventDefault(); } }; document.addEventListener("keydown", h); return () => document.removeEventListener("keydown", h); }, [mode, wsMode, advGroups, advSelId]);
  ...
  const renderAdvInlineQO = (g: AdvGroup) => {
    ...
    return (<div className="flex flex-col gap-4">{ds.multiSelect && <MultiSelectSection multiSelect={ds.multiSelect} values={msVals} onChange={(k, v) => upd(g.id, { multiSelectValues: { ...msVals, [k]: v } })} allowAllOff={ds.multiSelect.key === "algOptions"} />}{ds.multiSelect2 && <MultiSelectSection multiSelect={ds.multiSelect2} values={ms2Vals} onChange={(k, v) => upd(g.id, { multiSelectValues2: { ...ms2Vals, [k]: v } })} allowAllOff={ds.multiSelect2.key === "algOptions"} />}{ds.multiSelect3 && <MultiSelectSection multiSelect={ds.multiSelect3} values={ms3Vals} onChange={(k, v) => upd(g.id, { multiSelectValues3: { ...ms3Vals, [k]: v } })} />}{ds.variables.length > 0 && <VariablesSection variables={ds.variables} values={g.variables} onChange={(k, v) => upd(g.id, { variables: { ...g.variables, [k]: v } })} />}</div>);
  };

  const renderQCell = (q: AngleQuestion, globalIdx: number, localIdx: number, bgOvr?: string) => {
    const hh = 28, dh = q.tool === "rightAngle" ? CELL_H - hh : CELL_H; const ansText = q.answer2 ? `${q.answer}, ${q.answer2}` : q.answer;
    return (<div className="rounded-xl group" style={{ backgroundColor: bgOvr ?? stepBg, border: "1px solid #e5e7eb", overflow: "hidden", position: "relative" }}><span style={{ position: "absolute", top: 0, left: 0, fontSize: "0.65em", fontWeight: 700, color: "#000", lineHeight: 1, padding: "5px 5px 7px 5px", borderRight: "1px solid #000", borderBottom: "1px solid #000", zIndex: 5 }}>{localIdx + 1})</span>{q._qo && <button onClick={() => regenQ(globalIdx)} title="Regenerate" className="absolute top-1 right-1 w-6 h-6 rounded flex items-center justify-center text-gray-300 hover:text-blue-600 hover:bg-blue-50 transition-all opacity-0 group-hover:opacity-100" style={{ zIndex: 10 }}><RefreshCw size={12} /></button>}<div style={{ height: dh, display: "flex", alignItems: "center", justifyContent: "center", padding: "4px" }}><div style={{ width: "95%", height: "95%", display: "flex", alignItems: "center", justifyContent: "center" }}><AngleDiagram q={q} showAnswer={showWSAns} small dataIndex={globalIdx} /></div></div>{showWSAns && <div className="text-base font-bold text-center pb-2" style={{ color: "#059669" }}>{ansText}</div>}{q.tool === "rightAngle" && <div className="text-xs text-center pb-2 text-gray-500 italic">The diagram depicts a right angle</div>}</div>);
  };
  ...
  const renderAdvWS = () => {
    ...
    return (<div className="flex gap-3" style={{ minHeight: 300 }}><div className="flex flex-col rounded-xl border-2 border-gray-300 overflow-hidden" style={{ width: "50%", flexShrink: 0, backgroundColor: "#fff" }}><div className="flex-1 divide-y divide-gray-100 overflow-y-auto">{advGroups.map((g, idx) => { const isSel = g.id === advSelId; return (<div key={g.id} onClick={() => setAdvSelId(g.id)} className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50" style={{ borderLeft: `3px solid ${isSel ? lvBrd(g.level) : "transparent"}`, backgroundColor: isSel ? "#f0f4ff" : undefined }}><span className="text-xs font-bold text-gray-300 w-4 flex-shrink-0">{idx + 1}</span><div className="flex rounded-lg border-2 border-gray-200 overflow-hidden flex-shrink-0" onClick={e => e.stopPropagation()}>{(["level1","level2","level3"] as DifficultyLevel[]).map((lv, li) => <button key={lv} onClick={() => { upd(g.id, { ...mkAdvGroup(g.id, lv), id: g.id }); setAdvSelId(g.id); }} className={`px-2.5 py-1 font-bold text-xs ${g.level === lv ? `${lvCol(lv)} text-white` : "bg-white text-gray-400 hover:bg-gray-50"}`}>L{li + 1}</button>)}</div><div className="flex-1" /><div className="flex items-center gap-1 bg-gray-100 rounded-lg p-0.5 flex-shrink-0" onClick={e => e.stopPropagation()}><button onClick={() => upd(g.id, { count: Math.max(1, g.count - 1) })} disabled={g.count <= 1} className="w-6 h-6 flex items-center justify-center rounded-md text-gray-600 hover:bg-white disabled:opacity-30 font-bold leading-none">&#8722;</button><span className="w-6 text-center text-xs font-bold text-gray-800">{g.count}</span><button onClick={() => upd(g.id, { count: Math.min(24, g.count + 1) })} disabled={g.count >= 24} className="w-6 h-6 flex items-center justify-center rounded-md text-gray-600 hover:bg-white disabled:opacity-30 font-bold leading-none">&#43;</button></div>{advGroups.length > 1 && <button onClick={e => { e.stopPropagation(); const rem = advGroups.filter((_, i) => i !== idx); setAdvGroups(rem); if (g.id === advSelId) setAdvSelId(rem[Math.max(0, idx - 1)].id); }} className="w-6 h-6 rounded-full flex items-center justify-center text-gray-300 hover:bg-red-50 hover:text-red-400 flex-shrink-0"><X size={12} /></button>}</div>); })}</div><div className="px-4 py-3 border-t border-gray-200">{advGroups.length < 10 ? <button onClick={() => { const id = advNextId.current++; setAdvGroups(g => [...g, mkAdvGroup(id)]); setAdvSelId(id); }} className="w-full py-2 rounded-lg border-2 border-dashed border-gray-200 text-xs font-bold text-gray-400 hover:border-blue-300 hover:text-blue-600">+ Add group</button> : <p className="text-center text-xs text-gray-400 py-1">Max 10 groups</p>}</div></div><div className="flex-1 rounded-xl border-2 border-gray-300 px-5 py-4 overflow-y-auto" style={{ backgroundColor: "#fff" }}>{sel && (<><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Group {advGroups.indexOf(sel) + 1} &middot; {LV_LABELS[sel.level]} &middot; Options</p>{renderAdvInlineQO(sel)}</>)}</div></div>);
  };

  const renderControlBar = () => {
    if (mode === "worksheet") {
      const isAdv = wsMode === "advanced";
      return (<div className="bg-white rounded-xl shadow-lg mb-8"><div className="flex items-center gap-3 px-6 pt-4 pb-0"><label className="flex items-center gap-2 cursor-pointer"><div onClick={() => setWsMode(isAdv ? "standard" : "advanced")} className={`w-11 h-6 rounded-full transition-colors relative ${isAdv ? "bg-blue-900" : "bg-gray-300"}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${isAdv ? "translate-x-6" : "translate-x-1"}`} /></div><span className="text-sm font-bold text-gray-500">Advanced</span></label>{isAdv && <div className="ml-auto flex items-center gap-4"><label className="flex items-center gap-2 cursor-pointer"><div onClick={() => setAdvShuffle(s => !s)} className={`w-9 h-5 rounded-full relative ${advShuffle ? "bg-blue-900" : "bg-gray-300"}`}><div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${advShuffle ? "translate-x-4" : "translate-x-0.5"}`} /></div><span className="text-sm font-semibold text-gray-500">Shuffle</span></label><span className="text-sm font-bold text-gray-600">{totalAdv} questions total</span></div>}</div>{!isAdv ? (<div className="p-6"><div className="flex justify-center items-center gap-6 mb-4"><div className="flex rounded-xl border-2 border-gray-300 overflow-hidden shadow-sm">{[["level1","Level 1","bg-green-600"],["level2","Level 2","bg-yellow-500"],["level3","Level 3","bg-red-600"]].map(([val,lbl,col]) => <button key={val} onClick={() => { setDifficulty(val as DifficultyLevel); setIsDiff(false); }} className={`px-5 py-2 font-bold text-base ${!isDiff && difficulty === val ? `${col} text-white` : "bg-white text-gray-500 hover:bg-gray-50"}`}>{lbl}</button>)}</div><button onClick={() => setIsDiff(!isDiff)} className={`px-6 py-2 rounded-xl font-bold text-base shadow-sm border-2 ${isDiff ? "bg-blue-900 text-white border-blue-900" : "bg-white text-gray-600 border-gray-300 hover:border-blue-900 hover:text-blue-900"}`}>Differentiated</button></div><div className="flex justify-center items-center gap-6 mb-4">{isDiff ? <DiffQOPopover {...diffQO} /> : <StandardQOPopover {...stdQO} />}<div className="flex items-center gap-3"><label className="text-base font-semibold text-gray-700">Questions:</label><input type="number" min="1" max="24" value={numQ} onChange={e => setNumQ(Math.max(1, Math.min(24, parseInt(e.target.value) || 9)))} className="w-20 px-4 py-2 border-2 border-gray-300 rounded-lg text-base font-semibold text-center" /></div><div className="flex items-center gap-3"><label className="text-base font-semibold text-gray-700">Columns:</label><input type="number" min="1" max="4" value={isDiff ? 3 : numCols} onChange={e => { if (!isDiff) setNumCols(Math.max(1, Math.min(4, parseInt(e.target.value) || 3))); }} disabled={isDiff} className={`w-20 px-4 py-2 border-2 rounded-lg text-base font-semibold text-center ${isDiff ? "border-gray-200 bg-gray-100 text-gray-400 cursor-not-allowed" : "border-gray-300 bg-white"}`} /></div></div><div className="flex justify-center items-center gap-4"><button onClick={genWorksheet} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-base shadow-sm hover:bg-blue-800 flex items-center gap-2"><RefreshCw size={18} /> Generate</button>{worksheet.length > 0 && <><button onClick={() => setShowWSAns(!showWSAns)} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-base shadow-sm hover:bg-blue-800 flex items-center gap-2"><Eye size={18} /> {showWSAns ? "Hide Answers" : "Show Answers"}</button><button onClick={() => handlePrint(worksheet, isDiff, TOOL_CONFIG.tools[currentTool].name, wsRef)} className="px-6 py-2 bg-green-700 text-white rounded-xl font-bold text-base shadow-sm hover:bg-green-800 flex items-center gap-2"><Printer size={18} /> Print / PDF</button></>}</div></div>) : (<div className="p-6 pt-4">{renderAdvWS()}<div className="flex justify-center items-center gap-4 flex-wrap mt-4"><div className="flex items-center gap-2"><label className="text-base font-semibold text-gray-700">Columns:</label><input type="number" min="1" max="4" value={numCols} onChange={e => setNumCols(Math.max(1, Math.min(4, parseInt(e.target.value) || 3)))} className="w-20 px-4 py-2 border-2 border-gray-300 rounded-lg text-base font-semibold text-center" /></div><button onClick={genAdvanced} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-base shadow-sm hover:bg-blue-800 flex items-center gap-2"><RefreshCw size={18} /> Generate</button>{worksheet.length > 0 && <><button onClick={() => setShowWSAns(!showWSAns)} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-base shadow-sm hover:bg-blue-800 flex items-center gap-2"><Eye size={18} /> {showWSAns ? "Hide Answers" : "Show Answers"}</button><button onClick={() => handlePrint(worksheet, false, TOOL_CONFIG.tools[currentTool].name, wsRef)} className="px-6 py-2 bg-green-700 text-white rounded-xl font-bold text-base shadow-sm hover:bg-green-800 flex items-center gap-2"><Printer size={18} /> Print / PDF</button></>}</div></div>)}</div>);
    }
    return (<div className="px-5 py-4 rounded-xl" style={{ backgroundColor: qBg }}><div className="flex items-center justify-between gap-4"><DiffToggle value={difficulty} onChange={v => setDifficulty(v as DifficultyLevel)} /><StandardQOPopover {...stdQO} /><div className="flex gap-3 items-center"><button onClick={newQ} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-base shadow-sm hover:bg-blue-800 flex items-center gap-2"><RefreshCw size={18} /> New Question</button><button onClick={() => mode === "whiteboard" ? setShowWBA(!showWBA) : setShowAns(!showAns)} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-base shadow-sm hover:bg-blue-800 flex items-center gap-2"><Eye size={18} /> {(mode === "whiteboard" ? showWBA : showAns) ? "Hide Answer" : "Show Answer"}</button></div></div></div>);
  };

  const renderWhiteboard = () => {
    const fsTB = (<div style={{ background: fsTBg, borderBottom: "2px solid #000", padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexShrink: 0, zIndex: 210 }}><DiffToggle value={difficulty} onChange={v => setDifficulty(v as DifficultyLevel)} /><StandardQOPopover {...stdQO} /><div style={{ display: "flex", gap: 12, alignItems: "center" }}><button onClick={newQ} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-base shadow-sm hover:bg-blue-800 flex items-center gap-2"><RefreshCw size={18} /> New Question</button><button onClick={() => setShowWBA(a => !a)} className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-base shadow-sm hover:bg-blue-800 flex items-center gap-2"><Eye size={18} /> {showWBA ? "Hide Answer" : "Show Answer"}</button></div></div>);
    const ansText = curQ.answer2 ? `${curQ.answer}, ${curQ.answer2}` : curQ.answer;
    const qPanel = (isFS: boolean) => (<div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, ...(isFS ? { width: `${splitPct}%`, height: "100%", backgroundColor: fsQBg, padding: 32, boxSizing: "border-box", flexShrink: 0, overflowY: "auto" } : { width: "480px", height: "100%", backgroundColor: stepBg, borderRadius: 12, padding: 24, flexShrink: 0 }) }}>{showWBA && <div className={`${displayFontSizes[displayFontSize]} font-bold`} style={{ color: "#166534" }}>{ansText}</div>}<div style={{ width: "100%", flex: 1, minHeight: 0, display: "flex", alignItems: "center", justifyContent: "center" }}><AngleDiagram q={curQ} showAnswer={showWBA} /></div>{curQ.tool === "rightAngle" && <div className="text-base italic text-gray-500 text-center pb-1">The diagram depicts a right angle</div>}</div>);
    const rPanel = (isFS: boolean) => (<div style={{ flex: 1, height: "100%", position: "relative", overflow: "hidden", backgroundColor: presenter ? "#000" : (isFS ? fsWBg : stepBg) }} className={isFS ? "" : "flex-1 rounded-xl"}>{presenter && (<><video ref={vidRef} autoPlay playsInline muted style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />{camErr && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.4)", fontSize: "0.85rem", padding: "2rem", textAlign: "center", zIndex: 1 }}>{camErr}</div>}</>)}<div style={{ position: "absolute", top: 10, right: 10, display: "flex", gap: 6, zIndex: 20 }}>{presenter ? (<div style={{ position: "relative" }} ref={camDDRef}><button onMouseDown={() => { didLP.current = false; lpTimer.current = setTimeout(() => { didLP.current = true; setCamDDOpen(o => !o); }, 500); }} onMouseUp={() => { if (lpTimer.current) clearTimeout(lpTimer.current); if (!didLP.current) setPresenter(false); }} onMouseLeave={() => { if (lpTimer.current) clearTimeout(lpTimer.current); }} style={{ background: "rgba(0,0,0,0.55)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, cursor: "pointer", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}><Video size={16} color="rgba(255,255,255,0.85)" /></button>{camDDOpen && (<div style={{ position: "absolute", top: 40, right: 0, background: "rgba(12,12,12,0.96)", borderRadius: 8, minWidth: 200, overflow: "hidden", zIndex: 30 }}><div style={{ padding: "6px 14px", fontSize: "0.55rem", color: "rgba(255,255,255,0.25)", textTransform: "uppercase" }}>Camera</div>{camDevices.map((d, i) => (<div key={d.deviceId} onClick={() => { setCamDDOpen(false); if (d.deviceId !== curCamId) startCam(d.deviceId); }} style={{ padding: "10px 14px", fontSize: "0.75rem", color: d.deviceId === curCamId ? "#60a5fa" : "rgba(255,255,255,0.65)", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }} onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,0.07)")} onMouseLeave={e => (e.currentTarget.style.background = "transparent")}><div style={{ width: 5, height: 5, borderRadius: "50%", background: d.deviceId === curCamId ? "#60a5fa" : "transparent", flexShrink: 0 }} />{d.label || `Camera ${i + 1}`}</div>))}</div>)}</div>) : (<button onClick={() => setPresenter(true)} style={{ background: "rgba(0,0,0,0.08)", border: "none", borderRadius: 8, cursor: "pointer", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}><Video size={16} color="#6b7280" /></button>)}<button onClick={() => setWbFS(f => !f)} style={{ background: wbFS ? "#374151" : "rgba(0,0,0,0.08)", border: "none", borderRadius: 8, cursor: "pointer", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>{wbFS ? <Minimize2 size={16} color="#fff" /> : <Maximize2 size={16} color="#6b7280" />}</button></div></div>);
    if (wbFS) return (<div style={{ position: "fixed", inset: 0, zIndex: 200, backgroundColor: fsTBg, display: "flex", flexDirection: "column" }}>{fsTB}<div ref={splitCRef} style={{ flex: 1, display: "flex", minHeight: 0 }}>{qPanel(true)}<div style={{ position: "relative", width: 2, backgroundColor: "#000", flexShrink: 0, cursor: "col-resize" }} onMouseDown={e => { e.preventDefault(); isDragging.current = true; const onM = (ev: MouseEvent) => { if (!isDragging.current || !splitCRef.current) return; const r = splitCRef.current.getBoundingClientRect(); const p = ((ev.clientX - r.left) / r.width) * 100; setSplitPct(Math.min(75, Math.max(25, p >= 38 && p <= 42 ? 40 : p))); }; const onU = () => { isDragging.current = false; document.removeEventListener("mousemove", onM); document.removeEventListener("mouseup", onU); }; document.addEventListener("mousemove", onM); document.addEventListener("mouseup", onU); }}><div style={{ position: "absolute", top: 0, bottom: 0, left: -5, width: 12, cursor: "col-resize" }} /></div>{rPanel(true)}</div></div>);
  ...
  const renderWorkedExample = () => {
    const ansText = curQ.answer2 ? `${curQ.answer}, ${curQ.answer2}` : curQ.answer;
    return (<div className="overflow-y-auto" style={{ maxHeight: "120vh" }}><div className="p-8" style={{ backgroundColor: qBg }}><div className="text-center mb-6"><div className="flex justify-center mb-4" style={{ width: 340, height: 340, margin: "0 auto" }}><AngleDiagram q={curQ} showAnswer={showAns} /></div>{curQ.tool === "rightAngle" && <div className="text-base italic text-gray-500 text-center mb-4">The diagram depicts a right angle</div>}{showAns && (<><div className="space-y-4 mt-4">{curQ.working.slice(0, -1).map((step, i) => (<div key={i} className="rounded-xl p-6" style={{ backgroundColor: stepBg }}><h4 className="text-xl font-bold mb-2" style={{ color: "#000" }}>Step {i + 1}</h4><p className={`${displayFontSizes[displayFontSize]} font-semibold`} style={{ color: "#000" }}>{step.text}</p></div>))}</div><div className="rounded-xl p-6 text-center mt-4" style={{ backgroundColor: stepBg }}><span className={`${displayFontSizes[displayFontSize]} font-bold`} style={{ color: "#166534" }}>{ansText}</span></div></>)}</div></div></div>);
  };
  ...
  const renderWorksheet = () => {
    ...
    if (isDiff) return (<div ref={wsRef} className="rounded-xl shadow-2xl p-8" style={{ backgroundColor: qBg }}><h2 className="text-3xl font-bold text-center mb-8" style={{ color: "#000" }}>{title} &#8212; Worksheet</h2><div className="grid grid-cols-3 gap-4" style={{ alignItems: "start" }}>{(["level1","level2","level3"] as DifficultyLevel[]).map((lv, li) => { const lqs = worksheet.filter(q => q.level === lv), c = LV_COLORS[lv]; const offset = ["level1","level2","level3"].slice(0, li).reduce((s, l) => s + worksheet.filter(q => q.level === l).length, 0); return (<div key={lv} className={`${c.bg} border-2 ${c.border} rounded-xl p-4`}><h3 className={`text-xl font-bold mb-4 text-center ${c.text}`}>Level {li + 1}</h3><div style={{ display: "grid", gridTemplateColumns: "1fr", gridAutoRows: "1fr", gap: "0.75rem" }}>{lqs.map((q, idx) => <div key={idx} style={{ minHeight: 0 }}>{renderQCell(q, offset + idx, idx, c.fill)}</div>)}</div></div>); })}</div></div>);
    return (<div ref={wsRef} className="rounded-xl shadow-2xl p-8" style={{ backgroundColor: qBg }}><h2 className="text-3xl font-bold text-center mb-8" style={{ color: "#000" }}>{title} &#8212; Worksheet</h2><div style={{ display: "grid", gridTemplateColumns: `repeat(${numCols}, 1fr)`, gridAutoRows: "1fr", gap: "1rem" }}>{worksheet.map((q, idx) => <div key={idx} style={{ minHeight: 0 }}>{renderQCell(q, idx, idx)}</div>)}</div></div>);
  };

  return (<>
    <div className="bg-blue-900 shadow-lg"><div className="max-w-6xl mx-auto px-8 py-4 flex justify-between items-center"><button onClick={() => { window.location.href = "/"; }} className="flex items-center gap-2 text-white hover:bg-blue-800 px-4 py-2 rounded-lg"><Home size={24} /><span className="font-semibold text-lg">Home</span></button><div className="relative"><button onClick={() => setMenuOpen(!menuOpen)} className="text-white hover:bg-blue-800 p-2 rounded-lg">{menuOpen ? <X size={28} /> : <Menu size={28} />}</button>{menuOpen && <MenuDropdown colorScheme={cs} setColorScheme={setCs} onClose={() => setMenuOpen(false)} onOpenInfo={() => setInfoOpen(true)} />}</div></div></div>
    ...
    <div className="min-h-screen p-8" style={{ backgroundColor: "#f5f3f0" }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-center gap-4 mb-6 flex-wrap">{toolKeys.map(k => <button key={k} onClick={() => { setCurrentTool(k); setWorksheet([]); advNextId.current = 2; setAdvGroups([mkAdvGroup(1, "level1", k)]); setAdvSelId(1); }} className={`px-8 py-4 rounded-xl font-bold text-xl transition-all shadow-xl ${currentTool === k ? "bg-blue-900 text-white" : "bg-white text-gray-800 hover:bg-gray-100 hover:text-blue-900"}`}>{TOOL_CONFIG.tools[k].name}</button>)}</div>
        <div className="flex justify-center mb-8"><div style={{ width: "90%", height: "2px", backgroundColor: "#d1d5db" }} /></div>
        <div className="flex justify-center gap-4 mb-8">{([["whiteboard","Whiteboard"],["single","Worked Example"],["worksheet","Worksheet"]] as const).map(([m, lbl]) => <button key={m} onClick={() => { setMode(m); setPresenter(false); setWbFS(false); }} className={`px-8 py-4 rounded-xl font-bold text-xl transition-all shadow-xl ${mode === m ? "bg-blue-900 text-white" : "bg-white text-gray-800 hover:bg-gray-100 hover:text-blue-900"}`}>{lbl}</button>)}</div>
