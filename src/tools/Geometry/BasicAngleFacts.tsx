...
// Pick x from a range — integer or 1dp decimal
...
// Active expression types from vars
...
// Split a total into n random parts each >= minPart, summing exactly to total
function splitIntoN(total: number, n: number, minPart: number, useDecimal: boolean): number[] | null {
  ...
  for (let i = 0; i < n - 1; i++) {
    const lo = minPart;
    const hi = remaining - (n - 1 - i) * minPart;
    if (lo > hi) return null;
    let part: number;
    if (useDecimal) {
      part = Math.round(rndDecimal(lo, hi) * 10) / 10;
    } else {
      part = rnd(Math.ceil(lo), Math.floor(hi));
    }
    parts.push(part);
...
// ─── GENERATION CORE ──────────────────────────────────────────────────────────
// Strategy: pick x FIRST, derive everything else exactly. No rounding after x.
//
// For expression questions:
//   1. Pick c (coefficient)
//   2. Pick x in range that keeps c*x inside [minExpr, maxExpr]
//   3. For "both"/"constant": k is picked as small integer, exprVal = c*x + k
//   4. known angles derived as totalAngle - exprVal, split into parts
//
// For level 3 (two algebraic parts):
//   1. Pick c1, c2, pick x
//   2. k = totalAngle - (c1+c2)*x  (or totalAngle - fixed - (c1+c2)*x for 3-part)
//   3. Guaranteed exact sum, no rounding needed

// ─── RIGHT ANGLE (total = 90) ─────────────────────────────────────────────────
...
function buildRightL1(vars: QOVars): AngleQuestion {
  ...
  if (exprType !== null) {
    let xVal = 0, c = 1, k = 0, exprVal = 0, knownVal = 0;
    let found = false;
    for (let att = 0; att < 300 && !found; att++) {
      ...
      const x = pickX(xMin, xMax, useDecimal);
      if (x === null) continue;
      const cx = Math.round(c * x * 10) / 10;
      k = (exprType === "constant" || exprType === "both") ? rnd(3, Math.min(15, TOTAL - cx - MIN_A)) : 0;
      if (k < 0) continue;
      exprVal = Math.round((cx + k) * 10) / 10;
      knownVal = Math.round((TOTAL - exprVal) * 10) / 10;
    ...
    const fs = knownFirst ? knownVal : exprVal;
    const ss = knownFirst ? exprVal : knownVal;
    const rayDeg = sectorStart + fs;
    const cxStr = `${Math.round(c * xVal * 10) / 10}`;
    return {
      tool: "rightAngle", level: "level1", rotationDeg: rot,
      showSquare: vars.showSquare !== false,
      ...
      working: [
        ...
        { text: `${knownVal}\u00b0 + ${xLabel} = 90\u00b0` },
        { text: `${xLabel} = ${exprVal}\u00b0` },
        ...(k !== 0 ? [{ text: `${c === 1 ? "" : c}x = ${cxStr}\u00b0` }] : []),
        { text: `x = ${xVal}\u00b0` },
      ],
  ...
  const known = Math.round((TOTAL - x) * 10) / 10;
  const knownFirst = rnd(0, 1) === 0;
  const fs = knownFirst ? known : x, ss = knownFirst ? x : known;
  const rayDeg = sectorStart + fs;
  return {
    tool: "rightAngle", level: "level1", rotationDeg: rot,
    showSquare: vars.showSquare !== false,
...
function buildRightL2(vars: QOVars): AngleQuestion {
  ...
  if (exprType !== null) {
    let xVal = 0, c = 1, k = 0, exprVal = 0, p1 = 0, p2 = 0;
    let found = false;
    for (let att = 0; att < 300 && !found; att++) {
      c = (exprType === "coefficient" || exprType === "both") ? rnd(2, 4) : 1;
      const xMax = (exprType === "coefficient") ? Math.floor(MAX_E / c) : Math.floor((MAX_E - 3) / c);
      ...
      if (k < 0) continue;
      exprVal = Math.round((cx + k) * 10) / 10;
      const knownTotal = Math.round((TOTAL - exprVal) * 10) / 10;
      if (knownTotal < MIN_P * 2) continue;
      const parts = splitIntoN(knownTotal, 2, MIN_P, useDecimal);
      if (!parts) continue;
      p1 = parts[0]; p2 = parts[1]; xVal = x; found = true;
    }
    if (!found) { c = 2; xVal = 15; k = 0; exprVal = 30; p1 = 30; p2 = 30; }
    const xLabel = exprLabel(c, k);
    const r1 = sectorStart + p1, r2 = sectorStart + p1 + p2;
    const cxStr = `${Math.round(c * xVal * 10) / 10}`;
    return {
  ...
  let xVal = 0, p1 = 0, p2 = 0;
  let found = false;
  for (let att = 0; att < 100 && !found; att++) {
    const x = pickX(MIN_E, MAX_E, useDecimal);
    if (x === null) continue;
    const parts = splitIntoN(Math.round((TOTAL - x) * 10) / 10, 2, MIN_P, useDecimal);
    if (!parts) continue;
    xVal = x; p1 = parts[0]; p2 = parts[1]; found = true;
  }
  if (!found) { xVal = 30; p1 = 30; p2 = 30; }
  const ui = rnd(0, 2);
  const all = ui === 0 ? [xVal, p1, p2] : ui === 1 ? [p1, xVal, p2] : [p1, p2, xVal];
  const r1 = sectorStart + all[0], r2 = sectorStart + all[0] + all[1];
  const ap: [number, number][] = [[sectorStart, r1], [r1, r2], [r2, sectorEnd]];
  return {
    tool: "rightAngle", level: "level2", rotationDeg: rot,
    showSquare: vars.showSquare !== false,
    segments: [{ angleDeg: sectorStart }, { angleDeg: r1 }, { angleDeg: r2 }, { angleDeg: sectorEnd }],
    ...
    working: [
      { text: "Angles in a right angle sum to 90\u00b0" },
      { text: `${all.filter((_, i) => i !== ui).join("\u00b0 + ")}\u00b0 + x = 90\u00b0` },
      { text: `${TOTAL - xVal}\u00b0 + x = 90\u00b0` },
      { text: `x = ${xVal}\u00b0` },
    ],
    id: Math.floor(Math.random() * 1e6),
  };
...
function buildRightL3(vars: QOVars): AngleQuestion {
  const useCoef = vars.useCoefficients === true;
  ...
  if (twoRegions) {
    let xVal = 0, c1 = 1, c2 = 1, k = 0;
    let found = false;
    for (let att = 0; att < 300 && !found; att++) {
      c1 = useCoef ? rnd(1, 3) : 1;
      c2 = useCoef ? rnd(1, 3) : 1;
      const tc = c1 + c2;
      const xMin = Math.ceil(10 / Math.min(c1, c2));
      const xMax = Math.floor((TOTAL - 10) / tc);
      if (xMax < xMin) continue;
      const x = rnd(xMin, xMax);
      const kv = TOTAL - tc * x;
      if (Math.abs(kv) > 40) continue;
      if (c1 * x < 10 || c2 * x + kv < 10) continue;
      xVal = x; k = kv; found = true;
    }
    if (!found) { c1 = 1; c2 = 1; xVal = 30; k = 30; }
    const a1 = c1 * xVal, a2 = c2 * xVal + k;
    ...
    return {
      ...
      angles: [
        { label: exprLabel(c1, 0), isUnknown: true, value: a1, arcFromDeg: sectorStart, arcToDeg: sectorStart + a1, bisectorDeg: sectorStart + a1 / 2 },
        { label: exprLabel(c2, k), isUnknown: true, value: a2, arcFromDeg: sectorStart + a1, arcToDeg: sectorEnd, bisectorDeg: sectorStart + a1 + a2 / 2 },
      ],
      answer: `x = ${xVal}`,
      working: [
        { text: "Angles in a right angle sum to 90\u00b0" },
        { text: `${exprLabel(c1, 0)} + ${exprLabel(c2, k)} = 90\u00b0` },
        { text: `${tc}x ${kStr} = 90\u00b0` },
        { text: `${tc}x = ${TOTAL - k}\u00b0` },
        { text: `x = ${xVal}\u00b0` },
      ],
  ...
  let xVal = 0, c1 = 1, c2 = 1, fixed = 0, k = 0;
  let found = false;
  for (let att = 0; att < 300 && !found; att++) {
    c1 = useCoef ? rnd(1, 3) : 1;
    c2 = useCoef ? rnd(1, 3) : 1;
    ...
    const tc = c1 + c2;
    const xMin = Math.ceil(10 / Math.min(c1, c2));
    const xMax = Math.floor((rem - 10) / tc);
    if (xMax < xMin) continue;
    const x = rnd(xMin, xMax);
    const kv = rem - tc * x;
    if (Math.abs(kv) > 30) continue;
    if (c1 * x < 10 || c2 * x + kv < 10) continue;
    xVal = x; k = kv; found = true;
  }
  if (!found) { c1 = 1; c2 = 1; fixed = 20; xVal = 25; k = 20; }
  const a2 = c1 * xVal, a3 = c2 * xVal + k;
  const kStr = k >= 0 ? `+ ${k}` : `\u2212 ${Math.abs(k)}`;
  const tc = c1 + c2;
  ...
  return {
    ...
    segments: [{ angleDeg: sectorStart }, { angleDeg: r1 }, { angleDeg: r2 }, { angleDeg: sectorEnd }],
    angles: [
      { label: `${fixed}\u00b0`, isUnknown: false, value: fixed, arcFromDeg: sectorStart, arcToDeg: r1, bisectorDeg: sectorStart + fixed / 2 },
      { label: exprLabel(c1, 0), isUnknown: true, value: a2, arcFromDeg: r1, arcToDeg: r2, bisectorDeg: sectorStart + fixed + a2 / 2 },
      { label: exprLabel(c2, k), isUnknown: true, value: a3, arcFromDeg: r2, arcToDeg: sectorEnd, bisectorDeg: sectorStart + fixed + a2 + a3 / 2 },
    ],
    answer: `x = ${xVal}`,
    working: [
      { text: "Angles in a right angle sum to 90\u00b0" },
      { text: `${fixed}\u00b0 + ${exprLabel(c1, 0)} + ${exprLabel(c2, k)} = 90\u00b0` },
      { text: `${tc}x ${kStr} + ${fixed}\u00b0 = 90\u00b0` },
      { text: `${tc}x = ${TOTAL - fixed - k}\u00b0` },
      { text: `x = ${xVal}\u00b0` },
    ],
    id: Math.floor(Math.random() * 1e6),
  };
...
function buildStraightL1(vars: QOVars): AngleQuestion {
  ...
  const lld = rotationDeg + 180;
  const TOTAL = 180, MIN_A = 15, MIN_E = 20, MAX_E = 160;
  ...
  if (exprType !== null) {
    ...
    for (let att = 0; att < 300 && !found; att++) {
      c = (exprType === "coefficient" || exprType === "both") ? rnd(2, 4) : 1;
      const xMax = (exprType === "coefficient") ? Math.floor(MAX_E / c) : Math.floor((MAX_E - 5) / c);
      const xMin = Math.ceil(MIN_E / c);
      const x = pickX(xMin, xMax, useDecimal);
      if (x === null) continue;
      const cx = Math.round(c * x * 10) / 10;
      k = (exprType === "constant" || exprType === "both") ? rnd(5, Math.min(25, TOTAL - cx - MIN_A)) : 0;
      if (k < 0) continue;
      exprVal = Math.round((cx + k) * 10) / 10;
      knownVal = Math.round((TOTAL - exprVal) * 10) / 10;
      if (knownVal < MIN_A || exprVal < MIN_E || exprVal > MAX_E) continue;
    ...
    return {
      ...
      angles: [
        { label: knownFirst ? `${knownVal}\u00b0` : xLabel, isUnknown: !knownFirst, value: fs, arcFromDeg: lld, arcToDeg: rayDeg, bisectorDeg: lld + fs / 2 },
        { label: knownFirst ? xLabel : `${knownVal}\u00b0`, isUnknown: knownFirst, value: ss, arcFromDeg: rayDeg, arcToDeg: (rotationDeg as number) + 360, bisectorDeg: lld + fs + ss / 2 },
      ],
      answer: `x = ${xVal}\u00b0`,
      working: [
        { text: "Angles on a straight line sum to 180\u00b0" },
        { text: `${knownVal}\u00b0 + ${xLabel} = 180\u00b0` },
        { text: `${xLabel} = ${exprVal}\u00b0` },
        ...(k !== 0 ? [{ text: `${c === 1 ? "" : c}x = ${cxStr}\u00b0` }] : []),
        { text: `x = ${xVal}\u00b0` },
      ],
  ...
  const x = pickX(MIN_A, TOTAL - MIN_A, useDecimal) ?? 90;
  const known = Math.round((TOTAL - x) * 10) / 10;
  const knownFirst = rnd(0, 1) === 0;
  const fs = knownFirst ? known : x, ss = knownFirst ? x : known;
  const rayDeg = lld + fs;
  return {
    ...
    segments: [{ angleDeg: lld }, { angleDeg: rayDeg }, { angleDeg: rotationDeg as number }],
    angles: [
      { label: knownFirst ? `${known}\u00b0` : "x", isUnknown: !knownFirst, value: fs, arcFromDeg: lld, arcToDeg: rayDeg, bisectorDeg: lld + fs / 2 },
      { label: knownFirst ? "x" : `${known}\u00b0`, isUnknown: knownFirst, value: ss, arcFromDeg: rayDeg, arcToDeg: (rotationDeg as number) + 360, bisectorDeg: lld + fs + ss / 2 },
    ],
    answer: `x = ${x}\u00b0`,
...
function buildStraightL2(vars: QOVars): AngleQuestion {
  ...
  const rotationDeg = vars.fixedRotation ? 0 : rotations[rnd(0, 3)];
  const lld = rotationDeg + 180;
  const TOTAL = 180, MIN_P = 15, MIN_E = 20, MAX_E = 140;
  ...
  if (exprType !== null) {
    ...
    for (let att = 0; att < 300 && !found; att++) {
      ...
      if (x === null) continue;
      const cx = Math.round(c * x * 10) / 10;
      k = (exprType === "constant" || exprType === "both") ? rnd(5, Math.min(25, TOTAL - cx - MIN_P * 2)) : 0;
      if (k < 0) continue;
      exprVal = Math.round((cx + k) * 10) / 10;
      const kt = Math.round((TOTAL - exprVal) * 10) / 10;
      if (kt < MIN_P * 2) continue;
      const parts = splitIntoN(kt, 2, MIN_P, useDecimal);
      if (!parts) continue;
      p1 = parts[0]; p2 = parts[1]; xVal = x; found = true;
    }
    if (!found) { c = 2; xVal = 30; k = 0; exprVal = 60; p1 = 60; p2 = 60; }
    const xLabel = exprLabel(c, k);
    const r1 = lld + p1, r2 = lld + p1 + p2;
    const cxStr = `${Math.round(c * xVal * 10) / 10}`;
    return {
      ...
      angles: [
        { label: `${p1}\u00b0`, isUnknown: false, value: p1, arcFromDeg: lld, arcToDeg: r1, bisectorDeg: lld + p1 / 2 },
        { label: `${p2}\u00b0`, isUnknown: false, value: p2, arcFromDeg: r1, arcToDeg: r2, bisectorDeg: lld + p1 + p2 / 2 },
        { label: xLabel, isUnknown: true, value: exprVal, arcFromDeg: r2, arcToDeg: (rotationDeg as number) + 360, bisectorDeg: lld + p1 + p2 + exprVal / 2 },
      ],
      answer: `x = ${xVal}\u00b0`,
      working: [
        { text: "Angles on a straight line sum to 180\u00b0" },
        { text: `${p1}\u00b0 + ${p2}\u00b0 + ${xLabel} = 180\u00b0` },
        { text: `${p1 + p2}\u00b0 + ${xLabel} = 180\u00b0` },
        { text: `${xLabel} = ${exprVal}\u00b0` },
        ...(k !== 0 ? [{ text: `${c === 1 ? "" : c}x = ${cxStr}\u00b0` }] : []),
        { text: `x = ${xVal}\u00b0` },
  ...
  const ui = rnd(0, 2);
  const all = ui === 0 ? [xVal, p1, p2] : ui === 1 ? [p1, xVal, p2] : [p1, p2, xVal];
  const r1 = lld + all[0], r2 = lld + all[0] + all[1];
  const rd = rotationDeg as number;
  const ap: [number, number][] = [[lld, r1], [r1, r2], [r2, rd + 360]];
  return {
    ...
    working: [
      { text: "Angles on a straight line sum to 180\u00b0" },
      { text: `${all.filter((_, i) => i !== ui).join("\u00b0 + ")}\u00b0 + x = 180\u00b0` },
      { text: `${TOTAL - xVal}\u00b0 + x = 180\u00b0` },
      { text: `x = ${xVal}\u00b0` },
    ],
    id: Math.floor(Math.random() * 1e6),
  };
...
function buildStraightL3(vars: QOVars): AngleQuestion {
  const useCoef = vars.useCoefficients === true;
  const rotations = [0, 45, 90, 135];
  const rd = (vars.fixedRotation ? 0 : rotations[rnd(0, 3)]) as number;
  const lld = rd + 180;
  const partsVar = (vars.parts as string) ?? "mixed";
  ...
  if (!twoLines) {
    // Two algebraic parts: c1*x + (c2*x + k) = 180, pick x derive k
    let xVal = 0, c1 = 1, c2 = 1, k = 0;
    let found = false;
    for (let att = 0; att < 300 && !found; att++) {
      c1 = useCoef ? rnd(1, 4) : 1;
      c2 = useCoef ? rnd(1, 4) : 1;
      const tc = c1 + c2;
      const xMin = Math.ceil(20 / Math.min(c1, c2));
      const xMax = Math.floor((TOTAL - 20) / tc);
      if (xMax < xMin) continue;
      const x = rnd(xMin, xMax);
      const kv = TOTAL - tc * x;
      if (Math.abs(kv) > 60) continue;
      if (c1 * x < 20 || c2 * x + kv < 20) continue;
      xVal = x; k = kv; found = true;
    }
    ...
    return {
      ...
      angles: [
        { label: exprLabel(c1, 0), isUnknown: true, value: a1, arcFromDeg: lld, arcToDeg: lld + a1, bisectorDeg: lld + a1 / 2 },
        { label: exprLabel(c2, k), isUnknown: true, value: a2, arcFromDeg: lld + a1, arcToDeg: rd + 360, bisectorDeg: lld + a1 + a2 / 2 },
      ],
      answer: `x = ${xVal}`,
      working: [
        { text: "Angles on a straight line sum to 180\u00b0" },
        { text: `${exprLabel(c1, 0)} + ${exprLabel(c2, k)} = 180\u00b0` },
        { text: `${tc}x ${kStr} = 180\u00b0` },
        { text: `${tc}x = ${TOTAL - k}\u00b0` },
        { text: `x = ${xVal}\u00b0` },
      ],
  ...
  // Three parts: fixed + c1*x + (c2*x + k) = 180, pick fixed + x, derive k
  let xVal = 0, c1 = 1, c2 = 1, fixed = 0, k = 0;
  let found = false;
  for (let att = 0; att < 300 && !found; att++) {
    c1 = useCoef ? rnd(1, 4) : 1;
    ...
    const tc = c1 + c2;
    const xMin = Math.ceil(20 / Math.min(c1, c2));
    const xMax = Math.floor((rem - 20) / tc);
    if (xMax < xMin) continue;
    const x = rnd(xMin, xMax);
    const kv = rem - tc * x;
    if (Math.abs(kv) > 50) continue;
    if (c1 * x < 20 || c2 * x + kv < 20) continue;
    xVal = x; k = kv; found = true;
  }
  ...
  return {
    ...
    segments: [{ angleDeg: lld }, { angleDeg: r1 }, { angleDeg: r2 }, { angleDeg: rd }],
    angles: [
      { label: `${fixed}\u00b0`, isUnknown: false, value: fixed, arcFromDeg: lld, arcToDeg: r1, bisectorDeg: lld + fixed / 2 },
      { label: exprLabel(c1, 0), isUnknown: true, value: a2, arcFromDeg: r1, arcToDeg: r2, bisectorDeg: lld + fixed + a2 / 2 },
      { label: exprLabel(c2, k), isUnknown: true, value: a3, arcFromDeg: r2, arcToDeg: rd + 360, bisectorDeg: lld + fixed + a2 + a3 / 2 },
    ],
    answer: `x = ${xVal}`,
    working: [
      { text: "Angles on a straight line sum to 180\u00b0" },
      { text: `${fixed}\u00b0 + ${exprLabel(c1, 0)} + ${exprLabel(c2, k)} = 180\u00b0` },
      { text: `${tc}x ${kStr} + ${fixed}\u00b0 = 180\u00b0` },
      { text: `${tc}x = ${TOTAL - fixed - k}\u00b0` },
      { text: `x = ${xVal}\u00b0` },
    ],
    id: Math.floor(Math.random() * 1e6),
...
// ─── AROUND A POINT (total = 360) ─────────────────────────────────────────────
function buildAroundL1(vars: QOVars): AngleQuestion {
  const useDecimal = vars.numberType === "decimal";
  const exprType = pickExprType(vars);
  const sd = 270;
  const TOTAL = 360, MIN_P = 25, MIN_E = 30, MAX_E = 280;
  ...
  if (exprType !== null) {
    ...
    for (let att = 0; att < 300 && !found; att++) {
      ...
      if (x === null) continue;
      const cx = Math.round(c * x * 10) / 10;
      k = (exprType === "constant" || exprType === "both") ? rnd(5, Math.min(30, TOTAL - cx - MIN_P * 2)) : 0;
      if (k < 0) continue;
      exprVal = Math.round((cx + k) * 10) / 10;
      const kt = Math.round((TOTAL - exprVal) * 10) / 10;
      if (kt < MIN_P * 2) continue;
    ...
    return {
      ...
      angles: [
        { label: `${p1}\u00b0`, isUnknown: false, value: p1, arcFromDeg: sd, arcToDeg: r1, bisectorDeg: sd + p1 / 2 },
        { label: `${p2}\u00b0`, isUnknown: false, value: p2, arcFromDeg: r1, arcToDeg: r2, bisectorDeg: sd + p1 + p2 / 2 },
        { label: xLabel, isUnknown: true, value: exprVal, arcFromDeg: r2, arcToDeg: sd + 360, bisectorDeg: sd + p1 + p2 + exprVal / 2 },
      ],
      answer: `x = ${xVal}\u00b0`,
      working: [
        { text: "Angles around a point sum to 360\u00b0" },
        { text: `${p1}\u00b0 + ${p2}\u00b0 + ${xLabel} = 360\u00b0` },
        { text: `${p1 + p2}\u00b0 + ${xLabel} = 360\u00b0` },
        { text: `${xLabel} = ${exprVal}\u00b0` },
        ...(k !== 0 ? [{ text: `${c === 1 ? "" : c}x = ${cxStr}\u00b0` }] : []),
        { text: `x = ${xVal}\u00b0` },
  ...
  return {
    ...
    angles: [
      { label: `${p1}\u00b0`, isUnknown: false, value: p1, arcFromDeg: sd, arcToDeg: r1, bisectorDeg: sd + p1 / 2 },
      { label: `${p2}\u00b0`, isUnknown: false, value: p2, arcFromDeg: r1, arcToDeg: r2, bisectorDeg: sd + p1 + p2 / 2 },
      { label: "x", isUnknown: true, value: xVal, arcFromDeg: r2, arcToDeg: sd + 360, bisectorDeg: sd + p1 + p2 + xVal / 2 },
    ],
    answer: `x = ${xVal}\u00b0`,
    working: [
      { text: "Angles around a point sum to 360\u00b0" },
...
function buildAroundL2(vars: QOVars): AngleQuestion {
  ...
  const exprType = pickExprType(vars);
  const sd = 270;
  const TOTAL = 360, MIN_P = 25, MIN_E = 30, MAX_E = 240;

  if (exprType !== null) {
    let xVal = 0, c = 1, k = 0, exprVal = 0, p1 = 0, p2 = 0, p3 = 0;
    let found = false;
    for (let att = 0; att < 300 && !found; att++) {
      c = (exprType === "coefficient" || exprType === "both") ? rnd(2, 4) : 1;
      const xMax = (exprType === "coefficient") ? Math.floor(MAX_E / c) : Math.floor((MAX_E - 5) / c);
      ...
      if (x === null) continue;
      const cx = Math.round(c * x * 10) / 10;
      k = (exprType === "constant" || exprType === "both") ? rnd(5, Math.min(30, TOTAL - cx - MIN_P * 3)) : 0;
      if (k < 0) continue;
      exprVal = Math.round((cx + k) * 10) / 10;
      const kt = Math.round((TOTAL - exprVal) * 10) / 10;
      if (kt < MIN_P * 3) continue;
      const parts = splitIntoN(kt, 3, MIN_P, useDecimal);
      if (!parts) continue;
      p1 = parts[0]; p2 = parts[1]; p3 = parts[2]; xVal = x; found = true;
    }
    if (!found) { c = 2; xVal = 40; k = 0; exprVal = 80; p1 = 90; p2 = 100; p3 = 90; }
    const xLabel = exprLabel(c, k);
    const r1 = sd + p1, r2 = sd + p1 + p2, r3 = sd + p1 + p2 + p3;
    const cxStr = `${Math.round(c * xVal * 10) / 10}`;
    return {
      ...
      angles: [
        { label: `${p1}\u00b0`, isUnknown: false, value: p1, arcFromDeg: sd, arcToDeg: r1, bisectorDeg: sd + p1 / 2 },
        { label: `${p2}\u00b0`, isUnknown: false, value: p2, arcFromDeg: r1, arcToDeg: r2, bisectorDeg: sd + p1 + p2 / 2 },
        { label: `${p3}\u00b0`, isUnknown: false, value: p3, arcFromDeg: r2, arcToDeg: r3, bisectorDeg: sd + p1 + p2 + p3 / 2 },
        { label: xLabel, isUnknown: true, value: exprVal, arcFromDeg: r3, arcToDeg: sd + 360, bisectorDeg: sd + p1 + p2 + p3 + exprVal / 2 },
      ],
      answer: `x = ${xVal}\u00b0`,
      working: [
        { text: "Angles around a point sum to 360\u00b0" },
        { text: `${p1}\u00b0 + ${p2}\u00b0 + ${p3}\u00b0 + ${xLabel} = 360\u00b0` },
        { text: `${p1 + p2 + p3}\u00b0 + ${xLabel} = 360\u00b0` },
        { text: `${xLabel} = ${exprVal}\u00b0` },
        ...(k !== 0 ? [{ text: `${c === 1 ? "" : c}x = ${cxStr}\u00b0` }] : []),
        { text: `x = ${xVal}\u00b0` },
  ...
  let xVal = 0, p1 = 0, p2 = 0, p3 = 0;
  let found = false;
  for (let att = 0; att < 100 && !found; att++) {
    const x = pickX(MIN_E, MAX_E, useDecimal);
    if (x === null) continue;
    const parts = splitIntoN(Math.round((TOTAL - x) * 10) / 10, 3, MIN_P, useDecimal);
    if (!parts) continue;
    xVal = x; p1 = parts[0]; p2 = parts[1]; p3 = parts[2]; found = true;
  }
  if (!found) { xVal = 90; p1 = 90; p2 = 90; p3 = 90; }
  const ui = rnd(0, 3);
  const all = ui === 0 ? [xVal, p1, p2, p3]
            : ui === 1 ? [p1, xVal, p2, p3]
            : ui === 2 ? [p1, p2, xVal, p3]
            : [p1, p2, p3, xVal];
  let cum = 0;
  const rays = all.map(v => { const d = sd + cum; cum += v; return d; });
  const ap: [number, number][] = all.map((_, i) => [rays[i], i < all.length - 1 ? rays[i + 1] : sd + 360]);
  return {
    tool: "aroundPoint", level: "level2", rotationDeg: 0,
    ...
    working: [
      { text: "Angles around a point sum to 360\u00b0" },
      { text: `${all.filter((_, i) => i !== ui).join("\u00b0 + ")}\u00b0 + x = 360\u00b0` },
      { text: `${TOTAL - xVal}\u00b0 + x = 360\u00b0` },
      { text: `x = ${xVal}\u00b0` },
    ],
    id: Math.floor(Math.random() * 1e6),
  };
...
function buildAroundL3(vars: QOVars): AngleQuestion {
  ...
  if (twoRegions) {
    ...
    for (let att = 0; att < 300 && !found; att++) {
      ...
      c2 = useCoef ? rnd(1, 4) : 1;
      const tc = c1 + c2;
      const xMin = Math.ceil(30 / Math.min(c1, c2));
      const xMax = Math.floor((TOTAL - 30) / tc);
      if (xMax < xMin) continue;
      const x = rnd(xMin, xMax);
      const kv = TOTAL - tc * x;
      if (Math.abs(kv) > 80) continue;
      if (c1 * x < 30 || c2 * x + kv < 30) continue;
      xVal = x; k = kv; found = true;
    }
    ...
    return {
      ...
      angles: [
        { label: exprLabel(c1, 0), isUnknown: true, value: a1, arcFromDeg: sd, arcToDeg: sd + a1, bisectorDeg: sd + a1 / 2 },
        { label: exprLabel(c2, k), isUnknown: true, value: a2, arcFromDeg: sd + a1, arcToDeg: sd + 360, bisectorDeg: sd + a1 + a2 / 2 },
      ],
      answer: `x = ${xVal}`,
      working: [
        { text: "Angles around a point sum to 360\u00b0" },
        { text: `${exprLabel(c1, 0)} + ${exprLabel(c2, k)} = 360\u00b0` },
        { text: `${tc}x ${kStr} = 360\u00b0` },
        { text: `${tc}x = ${TOTAL - k}\u00b0` },
        { text: `x = ${xVal}\u00b0` },
      ],
  ...
  for (let att = 0; att < 300 && !found; att++) {
    ...
    const tc = c1 + c2;
    const xMin = Math.ceil(30 / Math.min(c1, c2));
    const xMax = Math.floor((rem - 30) / tc);
    if (xMax < xMin) continue;
    const x = rnd(xMin, xMax);
    const kv = rem - tc * x;
    if (Math.abs(kv) > 60) continue;
  ...
  return {
    ...
    segments: [{ angleDeg: sd }, { angleDeg: r1 }, { angleDeg: r2 }],
    angles: [
      { label: `${fixed}\u00b0`, isUnknown: false, value: fixed, arcFromDeg: sd, arcToDeg: r1, bisectorDeg: sd + fixed / 2 },
      { label: exprLabel(c1, 0), isUnknown: true, value: a2, arcFromDeg: r1, arcToDeg: r2, bisectorDeg: sd + fixed + a2 / 2 },
      { label: exprLabel(c2, k), isUnknown: true, value: a3, arcFromDeg: r2, arcToDeg: sd + 360, bisectorDeg: sd + fixed + a2 + a3 / 2 },
    ],
    answer: `x = ${xVal}`,
    working: [
      { text: "Angles around a point sum to 360\u00b0" },
      { text: `${fixed}\u00b0 + ${exprLabel(c1, 0)} + ${exprLabel(c2, k)} = 360\u00b0` },
      { text: `${tc}x ${kStr} + ${fixed}\u00b0 = 360\u00b0` },
      { text: `${tc}x = ${TOTAL - fixed - k}\u00b0` },
      { text: `x = ${xVal}\u00b0` },
    ],
    id: Math.floor(Math.random() * 1e6),
...
function buildVarsFromUI(tool: string, level: string, toolDropdowns: Record<string, string>, toolVariables: Record<string, Record<string, Record<string, boolean>>>, toolMultiSelect: Record<string, Record<string, boolean>>): QOVars {
  ...
  if (ds.dropdown) vars[ds.dropdown.key] = toolDropdowns[`${tool}__${level}`] ?? ds.dropdown.defaultValue;
  return vars;
}
...
function outwardBisectorDir(fromDeg: number, toDeg: number): [number, number] {
  ...
  const mid = fromDeg + sw / 2;
  return [Math.cos(toRad(mid)), Math.sin(toRad(mid))];
}
...
function AngleDiagram({ q, showAnswer, small = false, dataIndex }: DiagramProps) {
  ...
  if (q.tool === "rightAngle") {
    ...
    const bd = Math.max(bx1 - bx0, by1 - by0);
...
function handlePrint(worksheet: AngleQuestion[], isDiff: boolean, toolName: string, ref: React.RefObject<HTMLDivElement>) {
  ...
  const cell = (q: AngleQuestion, gi: number, li: number, showAns: boolean) =>
    `<div class="cell"><div class="cell-num">${li + 1}</div><div class="cell-diagram">${svgStrings[gi] ?? ""}</div>${showAns ? `<div class="answer">${q.answer}</div>` : ""}</div>`;
  ...
  const buildStd = (ans: boolean) => {
    const pages: string[] = [];
    for (let p = 0; p < worksheet.length; p += PRINT_PER_PAGE) {
      const sl = worksheet.slice(p, p + PRINT_PER_PAGE);
      const tp = Math.ceil(worksheet.length / PRINT_PER_PAGE);
      pages.push(stdPage(sl, p, Math.floor(p / PRINT_PER_PAGE) + 1, tp, ans));
    }
    return pages.join("");
  };
  const buildDiff = (ans: boolean) => {
    ...
    return Array.from({ length: tp }, (_, p) => {
      const cols = ["level1", "level2", "level3"].map((lv, li) => {
        const nc = ["#166534", "#854d0e", "#991b1b"][li], bg = ["#dcfce7", "#fef9c3", "#fee2e2"][li], lb = ["Level 1", "Level 2", "Level 3"][li];
        const qs = byLv[lv].slice(p * PRINT_ROWS, (p + 1) * PRINT_ROWS);
        const cells = qs.map((q, i) => cell(q, off[lv] + p * PRINT_ROWS + i, p * PRINT_ROWS + i, ans)).join("");
        return `<div class="dc"><div class="dh" style="color:${nc};background:${bg}">${lb}</div><div class="dcs">${cells}</div></div>`;
      }).join("");
...
const PopoverButton = ({ open, onClick }: { open: boolean; onClick: () => void }) => (
  <button onClick={onClick} className={`px-4 py-2 rounded-xl border-2 font-bold text-base transition-colors shadow-sm flex items-center gap-2 ${open ? "bg-blue-900 border-blue-900 text-white" : "bg-white border-gray-300 text-gray-600 hover:border-blue-900 hover:text-blue-900"}`}>
    Question Options <span style={{ display: "inline-block", transition: "transform 0.2s", transform: open ? "rotate(180deg)" : "rotate(0deg)" }}>&#9660;</span>
  </button>
);
...
interface QOProps { tool: ToolKey; level: DifficultyLevel; toolDropdowns: Record<string, string>; setToolDropdown: (t: string, l: string, v: string) => void; toolVariables: Record<string, Record<string, Record<string, boolean>>>; setToolVariable: (t: string, l: string, k: string, v: boolean) => void; toolMultiSelect: Record<string, Record<string, boolean>>; setToolMultiSelect: (t: string, l: string, k: string, v: boolean) => void; }

const StandardQOPopover = ({ tool, level, toolDropdowns, setToolDropdown, toolVariables, setToolVariable, toolMultiSelect, setToolMultiSelect }: QOProps) => {
  const { open, setOpen, ref } = usePopover();
  const t = TOOL_CONFIG.tools[tool], ds = t.difficultySettings[level];
  const ddVal = ds.dropdown ? (toolDropdowns[`${tool}__${level}`] ?? ds.dropdown.defaultValue) : "";
  const varVals = toolVariables[tool]?.[level] ?? {};
  const msVals: Record<string, boolean> = {}; if (ds.multiSelect) ds.multiSelect.options.forEach(o => { msVals[o.value] = toolMultiSelect[tool]?.[`${level}__${o.value}`] ?? o.defaultActive; });
  const ms2Vals: Record<string, boolean> = {}; if (ds.multiSelect2) ds.multiSelect2.options.forEach(o => { ms2Vals[o.value] = toolMultiSelect[tool]?.[`${level}__ms2__${o.value}`] ?? o.defaultActive; });
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 min-w-96 p-5 flex flex-col gap-5">
          {ds.multiSelect2 && <MultiSelectSection multiSelect={ds.multiSelect2} values={ms2Vals} onChange={(k, v) => setToolMultiSelect(tool, level, `ms2__${k}`, v)} />}
          {ds.dropdown && <DropdownSection dropdown={ds.dropdown} value={ddVal} onChange={v => setToolDropdown(tool, level, v)} />}
          {ds.multiSelect && <MultiSelectSection multiSelect={ds.multiSelect} values={msVals} onChange={(k, v) => setToolMultiSelect(tool, level, k, v)} allowAllOff={ds.multiSelect.key === "exprType"} />}
          {ds.variables.length > 0 && <VariablesSection variables={ds.variables} values={varVals} onChange={(k, v) => setToolVariable(tool, level, k, v)} />}
        </div>
      )}
    </div>
  );
...
interface DiffQOProps { tool: ToolKey; toolDropdowns: Record<string, string>; setToolDropdown: (t: string, l: string, v: string) => void; toolVariables: Record<string, Record<string, Record<string, boolean>>>; setToolVariable: (t: string, l: string, k: string, v: boolean) => void; toolMultiSelect: Record<string, Record<string, boolean>>; setToolMultiSelect: (t: string, l: string, k: string, v: boolean) => void; }

const DiffQOPopover = ({ tool, toolDropdowns, setToolDropdown, toolVariables, setToolVariable, toolMultiSelect, setToolMultiSelect }: DiffQOProps) => {
  const { open, setOpen, ref } = usePopover();
      ...
      {open && (
        <div className="absolute left-0 top-full mt-2 bg-white rounded-xl shadow-2xl border border-gray-200 z-50 min-w-96 p-5 flex flex-col gap-5">
          {(["level1", "level2", "level3"] as DifficultyLevel[]).map(lv => {
            ...
            const ddVal = ds.dropdown ? (toolDropdowns[`${tool}__${lv}`] ?? ds.dropdown.defaultValue) : "";
            ...
            return (
              <div key={lv} className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-3 pl-1">
                  {ds.multiSelect2 && <MultiSelectSection multiSelect={ds.multiSelect2} values={ms2Vals} onChange={(k, v) => setToolMultiSelect(tool, lv, `ms2__${k}`, v)} />}
                  {ds.dropdown && <DropdownSection dropdown={ds.dropdown} value={ddVal} onChange={v => setToolDropdown(tool, lv, v)} />}
                  {ds.multiSelect && <MultiSelectSection multiSelect={ds.multiSelect} values={msVals} onChange={(k, v) => setToolMultiSelect(tool, lv, k, v)} allowAllOff={ds.multiSelect.key === "exprType"} />}
                  {ds.variables.length > 0 && <VariablesSection variables={ds.variables} values={varVals} onChange={(k, v) => setToolVariable(tool, lv, k, v)} />}
                </div>
              </div>
            );
          })}
    ...
    <div ref={ref} className="absolute right-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden" style={{ minWidth: 200 }}>
      <div className="py-1">
        <button onClick={() => setColorOpen(!colorOpen)} className="w-full flex items-center justify-between px-4 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">
          <span>Colour Scheme</span><span className="text-xs text-gray-400 capitalize">{colorScheme}</span>
        </button>
...
export default function BasicAngleFacts() {
  ...
  const initDD = (): Record<string, string> => {
    const r: Record<string, string> = {};
    toolKeys.forEach(k => { (["level1","level2","level3"] as DifficultyLevel[]).forEach(lv => { const dd = TOOL_CONFIG.tools[k].difficultySettings[lv].dropdown; if (dd) r[`${k}__${lv}`] = dd.defaultValue; }); });
    return r;
  };
  ...
  const initMS = (): Record<string, Record<string, boolean>> => {
    ...
    toolKeys.forEach(k => { r[k] = {}; (["level1","level2","level3"] as DifficultyLevel[]).forEach(lv => { const ms = TOOL_CONFIG.tools[k].difficultySettings[lv].multiSelect; if (ms) ms.options.forEach(o => { r[k][`${lv}__${o.value}`] = o.defaultActive; }); const ms2 = TOOL_CONFIG.tools[k].difficultySettings[lv].multiSelect2; if (ms2) ms2.options.forEach(o => { r[k][`${lv}__ms2__${o.value}`] = o.defaultActive; }); }); });
    return r;
  };

  const [toolDropdowns, setToolDropdowns] = useState<Record<string, string>>(initDD);
  ...
  const setTD = (t: string, l: string, v: string) => setToolDropdowns(p => ({ ...p, [`${t}__${l}`]: v }));
  ...
  const bvars = (t: string, l: string) => buildVarsFromUI(t, l, toolDropdowns, toolVariables, toolMultiSelect);

  const [curQ, setCurQ] = useState<AngleQuestion>(() => generateQuestion("rightAngle", "level1", buildVarsFromUI("rightAngle", "level1", initDD(), initVars(), initMS())));
  ...
  const mkAdvGroup = (id: number, lv: DifficultyLevel = "level1", tk: ToolKey | null = null): AdvGroup => {
    ...
    return { id, level: lv, count: 5, variables: vars, dropdownValue: ds.dropdown?.defaultValue ?? "", multiSelectValues: ms, multiSelectValues2: ms2 };
  };
  ...
  const isDef = cs === "default";
  const fsTBg = isDef ? "#ffffff" : stepBg, fsQBg = isDef ? "#ffffff" : qBg, fsWBg = isDef ? "#f5f3f0" : qBg;

  const newQ = () => { setCurQ(generateQuestion(currentTool, difficulty, bvars(currentTool, difficulty))); setShowWBA(false); setShowAns(false); };
  ...
  const regenQ = (idx: number) => {
    ...
    for (let att = 0; att < 100; att++) {
      const c = generateQuestion(tool, level, vars);
      if (!existing.has(c.key ?? "")) { c._qo = JSON.parse(JSON.stringify(vars)); c._tool = tool; rep = c; break; }
    }
  ...
  const CELL_H = 280;
  const stdQO: QOProps = { tool: currentTool, level: difficulty, toolDropdowns, setToolDropdown: setTD, toolVariables, setToolVariable: setTV, toolMultiSelect, setToolMultiSelect: setTMS };
  const diffQO: DiffQOProps = { tool: currentTool, toolDropdowns, setToolDropdown: setTD, toolVariables, setToolVariable: setTV, toolMultiSelect, setToolMultiSelect: setTMS };

  const renderQCell = (q: AngleQuestion, idx: number, bgOvr?: string) => {
    ...
    return (
      <div className="rounded-xl group" style={{ backgroundColor: bgOvr ?? stepBg, border: "1px solid #e5e7eb", overflow: "hidden", position: "relative" }}>
        ...
        {q._qo && <button onClick={() => regenQ(idx)} title="Regenerate" className="absolute top-1 right-1 w-6 h-6 rounded flex items-center justify-center text-gray-300 hover:text-blue-600 hover:bg-blue-50 transition-all opacity-0 group-hover:opacity-100" style={{ zIndex: 10 }}><RefreshCw size={12} /></button>}
        <div style={{ height: dh, display: "flex", alignItems: "center", justifyContent: "center", padding: "4px" }}>
          <div style={{ width: "95%", height: "95%", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <AngleDiagram q={q} showAnswer={showWSAns} small dataIndex={idx} />
          </div>
        </div>
        {showWSAns && <div className="text-base font-bold text-center pb-2" style={{ color: "#059669" }}>{q.answer}</div>}
    ...
    return (
      <div className="flex gap-3" style={{ minHeight: 300 }}>
        ...
        <div className="flex-1 rounded-xl border-2 border-gray-300 px-5 py-4 overflow-y-auto" style={{ backgroundColor: "#fff" }}>
          {sel && (<><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4">Group {advGroups.indexOf(sel) + 1} &middot; {LV_LABELS[sel.level]} &middot; Options</p>
            <InlineQOPanel tool={currentTool} level={sel.level} variables={sel.variables} onVariableChange={(k, v) => upd(sel.id, { variables: { ...sel.variables, [k]: v } })} dropdownValue={sel.dropdownValue} onDropdownChange={v => upd(sel.id, { dropdownValue: v })} multiSelectValues={sel.multiSelectValues} onMultiSelectChange={(k, v) => upd(sel.id, { multiSelectValues: { ...sel.multiSelectValues, [k]: v } })} multiSelectValues2={sel.multiSelectValues2} onMultiSelectChange2={(k, v) => upd(sel.id, { multiSelectValues2: { ...sel.multiSelectValues2, [k]: v } })} /></>)}
        </div>
      </div>
    );
  };
        ...
        <div className="bg-white rounded-xl shadow-lg mb-8">
          <div className="flex items-center gap-3 px-6 pt-4 pb-0">
            ...
            {isAdv && <div className="ml-auto flex items-center gap-4">
              ...
              <span className="text-sm font-bold text-gray-600">{totalAdv} questions total</span>
            </div>}
          </div>
          {!isAdv ? (
            <div className="p-6">
  ...
  const renderWhiteboard = () => {
    ...
    const qPanel = (isFS: boolean) => (
      <div style={{ position: "relative", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, ...(isFS ? { width: `${splitPct}%`, height: "100%", backgroundColor: fsQBg, padding: 32, boxSizing: "border-box", flexShrink: 0, overflowY: "auto" } : { width: "480px", height: "100%", backgroundColor: stepBg, borderRadius: 12, padding: 24, flexShrink: 0 }) }}>
        {showWBA && <div className={`${displayFontSizes[displayFontSize]} font-bold`} style={{ color: "#166534" }}>{curQ.answer}</div>}
      ...
      <div className="p-8" style={{ backgroundColor: qBg }}>
        <div className="text-center mb-6">
          ...
          {showAns && (<>
            ...
            <div className="rounded-xl p-6 text-center mt-4" style={{ backgroundColor: stepBg }}><span className={`${displayFontSizes[displayFontSize]} font-bold`} style={{ color: "#166534" }}>{curQ.answer}</span></div>
          </>)}
        </div>
      </div>
    </div>
