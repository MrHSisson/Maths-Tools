...
import { RefreshCw, Eye, ChevronUp, ChevronDown, Home, Menu, X, Download } from "lucide-react";
...
const LEVEL_BG     = { level1:"#DCFCE7", level2:"#FEF9C3", level3:"#FEE2E2" };
const LEVEL_BORDER = { level1:"border-green-500", level2:"border-yellow-500", level3:"border-red-500" };
const LEVEL_LABEL  = { level1:"text-green-700",   level2:"text-yellow-700",   level3:"text-red-700" };
...
function diffBtnClass(idx, active) {
  if (active) return idx===0?"bg-green-600 text-white":idx===1?"bg-yellow-600 text-white":"bg-red-600 text-white";
...
// ─── PILL PLACEMENT ───────────────────────────────────────────────────────────
function pillCandidates(mx,my,isH,outDir,standoff){
...
function choosePillPositions(labels){
  const n=labels.length;if(!n)return[];
  const candidates=labels.map(l=>pillCandidates(l.mx,l.my,l.isH,l.outDir,l.standoff));
  function pillMinDist(ax,ay,pwa,pha,bx,by,pwb,phb){
    const dx=Math.abs(ax-bx)-(pwa+pwb)/2,dy=Math.abs(ay-by)-(pha+phb)/2;
  ...
  let best=new Array(n).fill(0),bs=-Infinity;
  const assign=new Array(n).fill(0);
  for(let t=0;t<Math.pow(3,n);t++){
    let tmp=t;for(let i=0;i<n;i++){assign[i]=tmp%3;tmp=Math.floor(tmp/3);}
    const s=score(assign);if(s>bs){bs=s;best=[...assign];}
  }
  return best.map((ci,i)=>({x:candidates[i][ci][0],y:candidates[i][ci][1]}));
}

function LabelPill({x,y,text,fontSize,color,pw,ph}){
  return(
    <g>
...
const L1_SHAPES=["equilateral_triangle","square","regular_pentagon","regular_hexagon","regular_octagon"];
const L2_SHAPES=["rectangle","isosceles_triangle","parallelogram","rhombus"];
...
function pickShapeKeys(levelOrLevels,count=9,maxPerFamily=2){
  const getLevelPool=lvl=>lvl==="level1"?L1_SHAPES:lvl==="level2"?L2_SHAPES:[...L1_SHAPES,...L2_SHAPES];
  const result=[],used=new Set(),familyCount={};
...
function polyRawPts(key,n,groupVals){
  if(key==="rectangle"){const w=groupVals[0],h=groupVals[1];return[[0,0],[w,0],[w,h],[0,h]];}
  if(key==="parallelogram"){const w=groupVals[0],h=groupVals[1],sh=h*0.4;return[[sh,0],[sh+w,0],[w,h],[0,h]];}
  if(key==="isosceles_triangle"){const leg=groupVals[0],base=groupVals[1],h=Math.sqrt(Math.max(1,leg*leg-(base/2)*(base/2)));return[[base/2,0],[base,h],[0,h]];}
  if(key==="rhombus"){const s=groupVals[0];return[[s*0.5,0],[s,s*0.6],[s*0.5,s*1.2],[0,s*0.6]];}
  const pts=[];for(let i=0;i<n;i++){const a=(i*2*Math.PI/n)-Math.PI/2;pts.push([Math.cos(a),Math.sin(a)]);}
  return pts;
}

function buildPolyQ(level,shapeKey=null){
  const mixUnits=level==="level3";
  const pool=mixUnits?[...L1_SHAPES,...L2_SHAPES]:level==="level1"?L1_SHAPES:L2_SHAPES;
  const key=shapeKey??pool[Math.floor(Math.random()*pool.length)];
  const def=POLY_SHAPES[key];
  const n=def.sides;
  let groupVals=def.groups.map(()=>rnd(3,20));
  if(["rectangle","parallelogram"].includes(key)){
    let att=0;
    while(Math.abs(groupVals[0]-groupVals[1])<3&&att++<20)groupVals=def.groups.map(()=>rnd(3,20));
    if(Math.abs(groupVals[0]-groupVals[1])<3)groupVals[1]=groupVals[0]>=18?groupVals[0]-5:groupVals[0]+5;
  }
  if(key==="isosceles_triangle"){
    const isValid=(leg,base)=>{if(base>=2*leg)return false;const h=Math.sqrt(leg*leg-(base/2)*(base/2));return(h/base)>=0.4;};
    let att=0;
    while(!isValid(groupVals[0],groupVals[1])&&att++<50)groupVals=def.groups.map(()=>rnd(4,16));
    if(!isValid(groupVals[0],groupVals[1])){const leg=rnd(8,14);const maxBase=Math.floor(Math.min(leg*1.2,2*leg-1));groupVals=[leg,rnd(Math.max(4,Math.ceil(leg*0.5)),maxBase)];}
  }
  const edges=Array(n);
  ...
  let displayEdges=null;
  const work=[];
  if(mixUnits){
    const useM=Math.random()<0.4;
    displayEdges=edges.map((e,i)=>{
      if(!labelledIdx.includes(i))return null;
      if(Math.random()<0.45){if(useM){const m=e/100;return{display:`${Number.isInteger(m)?m:m.toFixed(2)} m`,baseCm:e,unit:"m"};}return{display:`${e*10} mm`,baseCm:e,unit:"mm"};}
      return{display:`${e} cm`,baseCm:e,unit:"cm"};
    });
    const hasConv=displayEdges.some(d=>d&&d.unit!=="cm");
    if(hasConv){work.push({text:"Convert all measurements to centimetres:"});displayEdges.forEach(d=>{if(!d)return;if(d.unit==="mm")work.push({text:`${d.display} = ${d.baseCm} cm  (÷ 10)`});else if(d.unit==="m")work.push({text:`${d.display} = ${d.baseCm} cm  (× 100)`});});}
    ...
    work.push({text:`${def.name}: all ${n} sides are equal`});work.push({text:`Each side = ${groupVals[0]} cm`});
    work.push({text:`Perimeter = ${n} × ${groupVals[0]}`});work.push({text:`Perimeter = ${perimeter} cm`});
  } else {
    def.groups.forEach((grp,gi)=>{work.push({text:`${def.name} has ${grp.length} side${grp.length>1?"s":""} of ${groupVals[gi]} cm`});});
    work.push({text:`Perimeter = ${def.groups.map((g,gi)=>`${g.length} × ${groupVals[gi]}`).join(" + ")}`});
    work.push({text:`Perimeter = ${def.groups.map((g,gi)=>g.length*groupVals[gi]).join(" + ")}`});
...
function TickMarks({pts,groups,scale=1}){
...
function PolyDiagram({q,showAnswer,isWs=false,wsScale=1}){
  ...
  const labelledMeta=q.labelledIdx.map(i=>{
    ...
    return{mx,my,isH,outDir,standoff:adaptive,pw,ph,txt,color:"#065f46",edgeIdx:i};
  });
  ...
  return(
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{display:"block"}}>
      ...
      {q.labelledIdx.map(i=>{
        const p=pts[i],n=pts[(i+1)%pts.length];
        return <line key={`e${i}`} x1={p[0]} y1={p[1]} x2={n[0]} y2={n[1]} stroke="#10b981" strokeWidth={isWs?3:4} opacity={0.4}/>;
      })}
      {positions.map((pos,ri)=>{
        const l=labelledMeta[ri];
        return(<g key={`l${ri}`}>
          <line x1={l.mx} y1={l.my} x2={pos.x} y2={pos.y} stroke="#4b5563" strokeWidth="1.5" strokeDasharray="4,3"/>
          <LabelPill x={pos.x} y={pos.y} text={l.txt} fontSize={fs} color={l.color} pw={l.pw} ph={l.ph}/>
        </g>);
      })}
    </svg>
  );
...
// Shape template distribution — ensure no template repeats within a page of 9
function pickTemplateIndices(count=9){
  const total=RECT_TEMPLATES.length;
  const result=[];
  const used=new Set();
  for(let i=0;i<count;i++){
...
function genRect(templateIdx=null){
  const idx=templateIdx??Math.floor(Math.random()*RECT_TEMPLATES.length);
  const pts=RECT_TEMPLATES[idx]();
  const edges=pts.map((p,i)=>{const n=pts[(i+1)%pts.length];return Math.abs(n[0]-p[0])+Math.abs(n[1]-p[1]);});
  return{pts,edges,templateIdx:idx};
}
function pickHidden(pts,edges,count){
  const h=[],v=[];
  pts.forEach((p,i)=>{const n=pts[(i+1)%pts.length];(p[1]===n[1]?h:v).push(i);});
...
function deriveHidden(pts,edges,hi){
  const p1=pts[hi],p2=pts[(hi+1)%pts.length],isH=p1[1]===p2[1];
  let total=0,known=0;
  pts.forEach((p,i)=>{const n=pts[(i+1)%pts.length],h=p[1]===n[1];if(h===isH){total+=edges[i];if(i!==hi)known+=edges[i];}});
  return total-known;
}
function makeMixed(edges){
  const useM=Math.random()<0.4;
  return edges.map(e=>{
    if(Math.random()<0.4){if(useM){const m=e/100;return{display:`${Number.isInteger(m)?m:m.toFixed(2)} m`,baseCm:e,unit:"m"};}return{display:`${e*10} mm`,baseCm:e,unit:"mm"};}
    return{display:`${e} cm`,baseCm:e,unit:"cm"};
  });
}
function buildRectQ(level,templateIdx=null){
  const{pts,edges,templateIdx:usedIdx}=genRect(templateIdx);
  const perimeter=edges.reduce((s,e)=>s+e,0);
  if(level==="level1"){
    return{pts,edges,hiddenIdx:[],perimeter,displayAnswer:`${perimeter} cm`,
      working:[{text:"Add all edge lengths:"},{text:`Perimeter = ${edges.join(" + ")}`},{text:`Perimeter = ${perimeter} cm`}],
      mixedEdges:null,level:1,templateIdx:usedIdx};
  }
  if(level==="level2"){
    const hc=Math.random()<0.5?1:2,hidden=pickHidden(pts,edges,hc);
    const derivs=hidden.map(hi=>{const val=deriveHidden(pts,edges,hi);const p1=pts[hi],p2=pts[(hi+1)%pts.length],dir=p1[1]===p2[1]?"horizontal":"vertical";return{hi,val,dir};});
    const work=[];
    derivs.forEach(({hi,val,dir})=>{const known=edges.filter((_,i)=>{const p=pts[i],n=pts[(i+1)%pts.length];return(p[1]===n[1])===(pts[hi][1]===pts[(hi+1)%pts.length][1])&&i!==hi;});work.push({text:`Find the missing ${dir} edge:`});work.push({text:`Known ${dir} edges: ${known.join(" + ")} = ${known.reduce((s,v)=>s+v,0)} cm`});work.push({text:`Missing edge = ${val} cm`});});
    const full=edges.map((e,i)=>{const d=derivs.find(d=>d.hi===i);return d?d.val:e;});
    work.push({text:`Perimeter = ${full.join(" + ")}`});work.push({text:`Perimeter = ${perimeter} cm`});
  ...
  const work=[{text:"Convert all measurements to centimetres:"}];
  mixed.forEach((m,i)=>{if(m.unit==="mm")work.push({text:`Edge ${i+1}: ${m.display} = ${m.baseCm} cm  (÷ 10)`});else if(m.unit==="m")work.push({text:`Edge ${i+1}: ${m.display} = ${m.baseCm} cm  (× 100)`});});
  work.push({text:`Perimeter = ${edges.join(" + ")}`});work.push({text:`Perimeter = ${perimeter} cm`});
...
function RectDiagram({q,showAnswer,isWs=false,wsScale=1}){
  ...
  const sPts=q.pts.map(p=>[p[0]*sc+pad,p[1]*sc+pad]);
  const pathD=sPts.map((p,i)=>`${i===0?"M":"L"} ${p[0]} ${p[1]}`).join(" ")+" Z";
  const charW=fs*0.62,pillPad=fs*0.9;
  const labelMeta=sPts.map((p,i)=>{
    const n=sPts[(i+1)%sPts.length];
    const mx=(p[0]+n[0])/2,my=(p[1]+n[1])/2;
    const isH=Math.abs(p[1]-n[1])<0.01;
  ...
  return(
    <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{display:"block"}}>
      ...
      {sPts.map((p,i)=>{const n=sPts[(i+1)%sPts.length];return <line key={`e${i}`} x1={p[0]} y1={p[1]} x2={n[0]} y2={n[1]} stroke={labelMeta[i].hilit} strokeWidth={isWs?3:4} opacity={0.35}/>;})}
      {positions.map((pos,i)=>{
        const l=labelMeta[i];
        return(<g key={`l${i}`}>
          <line x1={l.mx} y1={l.my} x2={pos.x} y2={pos.y} stroke="#4b5563" strokeWidth="1.5" strokeDasharray="4,3"/>
...
const LABEL_H_MM=5,ANSWER_H_MM=5;

function pillWidthMM(text){return text.length*1.45+PILL_PAD_MM*2;}

function scaledPts(rawPts,cx,cy,availW,availH,frac=0.82){
...
function buildPillMetaMM(labelledIdx,groupVals,def,pts,cx,cy,standoff){
...
function drawTicksPDF(doc,pts,groups){
...
function drawGridPDF(doc,diffMode){
  ...
  if(diffMode){
    ...
    ["Level 1","Level 2","Level 3"].forEach((lbl,r)=>{doc.text(lbl,1.5,PDF_M+r*CELL_H+CELL_H/2,{angle:90,align:"center"});});
  }
}

// ── Polygon cell PDF ──────────────────────────────────────────────────────────
function drawPolyCellPDF(doc,q,cellX,cellY,showAnswer){
  ...
  const meta=buildPillMetaMM(q.labelledIdx,q.groupVals,q.def,pts,cx,cy,8.5);
...
// ── Rectilinear cell PDF ──────────────────────────────────────────────────────
function drawRectCellPDF(doc,q,cellX,cellY,showAnswer){
  const maxX=Math.max(...q.pts.map(p=>p[0])),maxY=Math.max(...q.pts.map(p=>p[1]));
  const availW=CELL_W-CPAD*2,availH=CELL_H-CPAD*2-LABEL_H_MM-(showAnswer?ANSWER_H_MM:0);
  ...
  const sPts=q.pts.map(p=>[offX+p[0]*sc,offY+p[1]*sc]);
  ...
  // pills for each edge
  const charW=1.45*0.72,pillPad=PILL_PAD_MM;
  const meta=sPts.map((p,i)=>{
    const n=sPts[(i+1)%sPts.length];
    const mx=(p[0]+n[0])/2,my=(p[1]+n[1])/2;
    const isH=Math.abs(p[1]-n[1])<0.01;
    const outDir=edgeOutDir(sPts,i);
    ...
    const pw=pillWidthMM(txt),ph=PILL_H_MM;
    return{mx,my,isH,outDir,standoff:8.5,pw,ph,txt,color:hidden?"#d97706":"#1e40af"};
  });
  ...
  pos.forEach((p,i)=>{
    ...
    doc.setFillColor(255,255,255);doc.setDrawColor(100,100,100);doc.setLineWidth(0.25);
    doc.roundedRect(p.x-l.pw/2,p.y-l.ph/2,l.pw,l.ph,l.ph/2,l.ph/2,"FD");
    doc.setFontSize(FONT_PT);doc.setTextColor(l.color==="#d97706"?[217,119,6]:[30,64,175]);
    doc.text(l.txt,p.x,p.y+0.15,{align:"center",baseline:"middle"});
  });
...
function renderPagePDF(doc,questions,showAnswer,diffMode,type){
  questions.forEach((q,i)=>{
    const row=Math.floor(i/PDF_COLS),col=i%PDF_COLS;
    if(type==="polygon") drawPolyCellPDF(doc,q,PDF_M+col*CELL_W,PDF_M+row*CELL_H,showAnswer);
    else                 drawRectCellPDF(doc,q,PDF_M+col*CELL_W,PDF_M+row*CELL_H,showAnswer);
  });
  drawGridPDF(doc,diffMode);
}

// ══════════════════════════════════════════════════════════════════════════════
// SHARED WORKSHEET PREVIEW (SVG-based)
// ══════════════════════════════════════════════════════════════════════════════
function WorksheetPreview({questions,diffMode,pageIndex,DiagramComponent}){
...
// ══════════════════════════════════════════════════════════════════════════════
function WorksheetPanel({col, buildQuestions, DiagramComponent, pdfType, filename}){
  ...
  function generate(){
    ...
    for(let p=0;p<pages;p++){
      const pageQs=buildQuestions(diff,diff2);
      qs.push(...pageQs);
    }
  ...
  async function downloadPDF(){
    ...
    }catch(e){setPdfStatus("❌ "+e.message);console.error(e);}
  }
  ...
  return(<>
    <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
      ...
      <div className="flex justify-center gap-4">
        ...
        {questions.length>0&&<button onClick={downloadPDF} className="px-6 py-3 bg-blue-900 text-white rounded-lg font-semibold flex items-center gap-2 hover:bg-blue-800"><Download size={20}/>Download PDF</button>}
      </div>
      {pdfStatus&&<p className="text-center mt-3 text-sm font-semibold text-gray-600">{pdfStatus}</p>}
    </div>
    {questions.length>0&&(
...
// ── Question builders for each subtool ───────────────────────────────────────
function buildPolyQuestions(diff,diff2){
  if(diff2){
...
function buildRectQuestions(diff,diff2){
...
// SHARED CONTROLS BAR
// ══════════════════════════════════════════════════════════════════════════════
function ControlsBar({difficulty,setDifficulty,onNew,onToggleAnswer,showAnswer}){
  return(
    <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
...
function RectWhiteboard({col}){
  ...
  useEffect(()=>{newQ();},[diff,missing1,missing2]);
  return(<>
    <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
...
// POLYGON PANELS
// ══════════════════════════════════════════════════════════════════════════════
function PolyWhiteboard({col}){
  const[diff,setDiff]=useState("level1");
  const[q,setQ]=useState(()=>buildPolyQ("level1"));
  const[show,setShow]=useState(false);
  ...
  return(<>
    ...
    <div className="min-h-screen p-8" style={{backgroundColor:"#f5f3f0"}}>
      <div className="max-w-6xl mx-auto">
        ...
        {subtool==="rectilinear"&&mode==="worksheet"  &&<WorksheetPanel col={col} buildQuestions={buildRectQuestions} DiagramComponent={RectDiagram} pdfType="rect" filename="perimeter-rectilinear.pdf"/>}
        {subtool==="polygons"   &&mode==="whiteboard" &&<PolyWhiteboard col={col}/>}
        {subtool==="polygons"   &&mode==="single"     &&<PolyWorked col={col}/>}
        {subtool==="polygons"   &&mode==="worksheet"  &&<WorksheetPanel col={col} buildQuestions={buildPolyQuestions} DiagramComponent={PolyDiagram} pdfType="polygon" filename="perimeter-polygons.pdf"/>}
      </div>
    </div>
  </>);
}
