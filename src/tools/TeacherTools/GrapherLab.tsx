  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-200 bg-white">
        ...
        <h1 className="text-xl font-bold text-slate-800">Grapher Lab</h1>
        ...
        <span className="ml-auto text-sm text-slate-400 hidden sm:block">Pick a scenario · edit its numbers · Expand for full interactivity</span>
      </div>
                ...
                {section.items.map((item) => (
                  <button
                    ...
                    className={`text-left px-2.5 py-1.5 rounded-lg text-sm transition-colors ${
                      isActive(item) ? "bg-blue-600 text-white font-medium" : "text-slate-600 hover:bg-slate-100"
                    }`}
                  >
                    {item.label}
                  </button>
