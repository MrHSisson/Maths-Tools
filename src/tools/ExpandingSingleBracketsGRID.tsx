...
export default function ExpandingSingleBracketsGridMethod() {
  ...
  const getRandomVariable = (): string => {
    const vars: string[] = ['x', 'y', 'a', 'b', 'p', 'q', 'r', 's', 't', 'n', 'm'];
  ...
  const TableDisplay = ({ q }: { q: Question | GridData }): JSX.Element => {
    ...
    if (q.withVariable) {
      const firstHeader: string = q.b === 1 ? v : q.b === -1 ? '−' + v : q.b + v;
      const secondHeader: string = q.c >= 0 ? '+' + q.c : String(q.c);
      
      let firstCell: string = '';
      let secondCell: string = '';
      let outsideCoeff: string = '';
      
      if (q.outsidePower === 2) {
        firstCell = q.x3Coeff === 1 ? v + '³' : q.x3Coeff === -1 ? '−' + v + '³' : q.x3Coeff + v + '³';
        secondCell = q.x2Coeff === 1 ? v + '²' : q.x2Coeff === -1 ? '−' + v + '²' : q.x2Coeff + v + '²';
        outsideCoeff = q.a === 1 ? v + '²' : q.a === -1 ? '−' + v + '²' : q.a + v + '²';
      } else {
        firstCell = q.x2Coeff === 1 ? v + '²' : q.x2Coeff === -1 ? '−' + v + '²' : q.x2Coeff + v + '²';
        secondCell = q.xCoeff === 1 ? v : q.xCoeff === -1 ? '−' + v : q.xCoeff + v;
        outsideCoeff = q.a === 1 ? v : q.a === -1 ? '−' + v : q.a + v;
      }
      
      return (
        <table className="border-collapse border-2 border-gray-800 mx-auto">
          <thead>
            <tr>
              <th className="border-2 border-gray-800 px-6 py-4 bg-gray-200 text-2xl font-bold">×</th>
              <th className="border-2 border-gray-800 px-6 py-4 bg-gray-200 text-2xl font-bold">{firstHeader}</th>
              <th className="border-2 border-gray-800 px-6 py-4 bg-gray-200 text-2xl font-bold">{secondHeader}</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="border-2 border-gray-800 px-6 py-4 bg-gray-200 text-2xl font-bold">{outsideCoeff}</td>
              <td className="border-2 border-gray-800 px-6 py-4 text-2xl font-semibold text-red-600">{firstCell}</td>
              <td className="border-2 border-gray-800 px-6 py-4 text-2xl font-semibold text-red-600">{secondCell}</td>
            </tr>
          </tbody>
        </table>
      );
    } else {
      const firstHeader: string = q.isReversed ? String(q.c) : (q.b === 1 ? v : q.b === -1 ? '−' + v : q.b + v);
      const secondHeader: string = q.isReversed ? '−' + (q.b === 1 ? '' : q.b) + v : (q.c >= 0 ? '+' + q.c : String(q.c));
      const firstCell: string = q.isReversed ? String(q.constant) : (q.xCoeff === 1 ? v : q.xCoeff === -1 ? '−' + v : q.xCoeff + v);
      const secondCell: string = q.isReversed ? (q.xCoeff === 1 ? v : q.xCoeff === -1 ? '−' + v : q.xCoeff + v) : String(q.constant);
      ...
      return (
        <table className="border-collapse border-2 border-gray-800 mx-auto">
          ...
          <tbody>
            <tr>
              <td className="border-2 border-gray-800 px-6 py-4 bg-gray-200 text-2xl font-bold">{q.a}</td>
              <td className="border-2 border-gray-800 px-6 py-4 text-2xl font-semibold text-red-600">{firstCell}</td>
              <td className="border-2 border-gray-800 px-6 py-4 text-2xl font-semibold text-red-600">{secondCell}</td>
            </tr>
          </tbody>
  ...
  const formatAnswer = (xCoeff: number, constant: number, varName: string): string => {
    const hasPositiveX: boolean = xCoeff > 0;
    const hasNegativeX: boolean = xCoeff < 0;
    const hasPositiveC: boolean = constant > 0;
    const hasNegativeC: boolean = constant < 0;
  ...
  const combineTerms = (q1: Question, q2: Question, operator: string, varName: string): string => {
    ...
    // Apply operator
    const multiplier: number = operator === '+' ? 1 : -1;
    
    const finalX3: number = x3_1 + multiplier * x3_2;
    const finalX2: number = x2_1 + multiplier * x2_2;
    const finalX: number = x_1 + multiplier * x_2;
    const finalC: number = c_1 + multiplier * c_2;
  ...
  const generateQuestion = (level: DifficultyLevel, multiplier: MultiplierType, forcedVarName: string | null = null): Question => {
    ...
    const varName: string = forcedVarName || getRandomVariable();
    const useVarThisTime: boolean = multiplier === 'algebraic' ? true : multiplier === 'mixed' ? Math.random() > 0.5 : false;
    const outsidePower: number = useVarThisTime ? (Math.random() > 0.5 ? 1 : 2) : 0;
    ...
    let original = '';
    if (useVarThisTime) {
      const aTerm: string = a === 1 ? varName + (outsidePower === 2 ? '²' : '') : 
                    a === -1 ? '−' + varName + (outsidePower === 2 ? '²' : '') : 
                    a + varName + (outsidePower === 2 ? '²' : '');
      const bTerm: string = b === 1 ? varName : b === -1 ? '−' + varName : b + varName;
      const cTerm: string = c === 0 ? '' : c > 0 ? ' + ' + c : ' − ' + Math.abs(c);
    ...
    const working: WorkingStep[] = [{
      type: 'grid',
      content: 'Use the grid method to multiply each term',
      gridData: { a, b, c, isReversed, withVariable: useVarThisTime, outsidePower, varName, xCoeff, x2Coeff, x3Coeff, constant }
    }];
  ...
  const generateSimplifyQuestion = (level: DifficultyLevel, multiplier: MultiplierType): Question => {
    const varName: string = getRandomVariable();
    const q1: Question = generateQuestion(level === 'level1' ? 'level1' : level === 'level2' ? 'level1' : 'level3', multiplier, varName);
    const q2: Question = generateQuestion(level === 'level1' ? 'level1' : level === 'level2' ? (Math.random() > 0.5 ? 'level1' : 'level2') : (Math.random() > 0.5 ? 'level1' : 'level2'), multiplier, varName);
    
    const operator: string = level === 'level1' ? '+' : (Math.random() > 0.5 ? '+' : '−');
    
    const original: string = q1.display + ' ' + operator + ' ' + q2.display;
    ...
    const finalAnswer: string = combineTerms(q1, q2, operator, varName);
    ...
    return {
      ...
      answer: finalAnswer,
      working,
      values: { a: q1.a, b: q1.b, c: q1.c, q1, q2, operator },
      difficulty: level,
      q1,
      q2,
      operator,
      ...
      <div className="min-h-screen p-8" style={{ backgroundColor: '#f5f3f0' }}>
        <div className="max-w-6xl mx-auto">
          ...
          <h1 className="text-5xl font-bold text-center mb-8" style={{ color: '#000000' }}>
            Expanding Single Brackets (Grid Method)
          </h1>
                            ...
                            <div key={i} className="rounded-xl p-6" style={{ backgroundColor: getStepBg() }}>
                              ...
                              {step.type === 'grid' ? (
                                <div>
                                  ...
                                  <TableDisplay q={step.gridData || currentQuestion} />
                                </div>
                              ) : (
              ...
              {worksheet.length > 0 && (
                <div className="rounded-xl shadow-2xl p-8 relative" style={{ backgroundColor: getQuestionBg() }}>
                  ...
                  <h2 className="text-3xl font-bold text-center mb-8" style={{ color: '#000000' }}>
                    Expanding Single Brackets (Grid Method) - Worksheet
                  </h2>
