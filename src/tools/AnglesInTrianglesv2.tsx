...
function buildSplitTriangle(vars: Record<string, unknown>): TriQuestion {
  ...
  if (variant === 1) {
    ...
    const dInterior = useLeft ? angBDA : angCDA;
    const xVal = 180 - dInterior;
...
// ─── DIAGRAM ─────────────────────────────────────────────────────────────────
// Each diagram rendered into the worksheet grid gets a data-q-index attribute
// so the print function can locate and serialise it by position.
...
interface DiagramProps {
  ...
  small?: boolean;
  labelBg?: string;
  // When set, written onto the root <svg> element for print lookup
  dataIndex?: number;
}
...
function TriangleDiagram({ q, showAnswer, small = false, labelBg = "#ffffff", dataIndex }: DiagramProps) {
  ...
  // Extra SVG props — attach data-q-index when rendering into the worksheet grid
  const extraSvgProps = dataIndex !== undefined ? { "data-q-index": dataIndex } : {};
        ...
        return (
          <g key={`lbl-${i}`}>
            <line
              ...
              stroke={colour} strokeWidth={small ? 1 : 1.5}
              strokeDasharray={small ? "3 2" : "5 3"} strokeLinecap="round"
            />
            ...
            <rect x={lp.x - tw / 2 - 4} y={lp.y - th / 2 - 2} width={tw + 8} height={th + 4} rx={4}
              fill={labelBg} fillOpacity="0.97" stroke="#000000" strokeWidth={0.5} />
...
// ─── PRINT ────────────────────────────────────────────────────────────────────
// Collects already-rendered SVGs from the DOM and builds a print-ready HTML page.
// Layout: always 3 columns, max 5 rows per page → 15 questions per page.
// For differentiated mode each level gets its own column (5 rows per level per page).

const PRINT_COLS = 3;
const PRINT_ROWS = 5;
const PRINT_PER_PAGE = PRINT_COLS * PRINT_ROWS; // 15
...
function handlePrint(
  ...
  svgEls.forEach(el => {
    ...
    // Ensure the cloned SVG fills its print cell via width/height 100%
    clone.setAttribute("width", "100%");
    clone.setAttribute("height", "100%");
    // Preserve viewBox so aspect ratio is correct
  ...
  // Build per-page HTML
  // Standard: slice worksheet into pages of PRINT_PER_PAGE
  // Differentiated: each page shows up to PRINT_ROWS per level, 3 levels side-by-side
  ...
  const buildStandardPages = (): string => {
    ...
    for (let p = 0; p < worksheet.length; p += PRINT_PER_PAGE) {
      ...
      const cells = slice.map((q, localIdx) => {
        ...
        const svg = svgStrings[globalIdx] ?? "";
        const answerHtml = showAnswers
          ? `<div class="answer">${q.answer}</div>`
          : "";
        return `
          <div class="cell">
            <div class="cell-num">${globalIdx + 1}</div>
            <div class="cell-diagram">${svg}</div>
            ${answerHtml}
          </div>`;
      }).join("");
      pages.push(`
        <div class="page">
          <div class="page-header">
            <span class="page-title">${toolName}</span>
            <span class="page-meta">${dateStr}${totalPages > 1 ? ` &nbsp;·&nbsp; Page ${pageNum} of ${totalPages}` : ""}</span>
          </div>
          <div class="grid standard-grid">${cells}</div>
        </div>`);
    }
    return pages.join("");
  };
  ...
  const buildDiffPages = (): string => {
    ...
    // Index into svgStrings: differentiated worksheet stores level1 first, then level2, then level3
    ...
    for (let p = 0; p < totalPages; p++) {
      ...
      const cols = ["level1", "level2", "level3"].map(lv => {
        ...
        const cells = qs.map((q, localIdx) => {
          ...
          const svg = svgStrings[globalIdx] ?? "";
          const answerHtml = showAnswers ? `<div class="answer">${q.answer}</div>` : "";
          return `
            <div class="cell">
              <div class="cell-num">${p * PRINT_ROWS + localIdx + 1}</div>
              <div class="cell-diagram">${svg}</div>
              ${answerHtml}
            </div>`;
        }).join("");
        return `
          <div class="diff-col">
            <div class="diff-col-header" style="color:${lvColors[lv]};background:${lvBg[lv]}">
              ${lvNames[lv]}
            </div>
      ...
      pages.push(`
        <div class="page">
          ...
          <div class="grid diff-grid">${cols}</div>
        </div>`);
    }
    return pages.join("");
  };

  const pagesHtml = isDiff ? buildDiffPages() : buildStandardPages();
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* ── Standard grid: 3 cols × 5 rows ── */
  /* usable height after header ≈ 268mm − 12mm header = 256mm; 5 rows with 2mm gap = (256 − 8) / 5 = 49.6mm */
  .standard-grid {
    display: grid;
  ...
  /* ── Differentiated grid: 3 equal columns ── */
  .diff-grid {
    display: grid;
    grid-template-columns: repeat(3, 1fr);
    gap: 2mm;
...
export default function AnglesInTriangleTool() {
  ...
  // Ref to the worksheet container — used by handlePrint to locate rendered SVGs
  ...
  const startCam = useCallback(async (deviceId?: string) => {
    ...
    try {
      ...
      if (!targetDeviceId) {
        ...
        const external = all.filter(d => d.kind === "videoinput").find(d => d.label && !builtInPattern.test(d.label));
  ...
  // ── Worksheet cell ──────────────────────────────────────────────────────────
  // Each cell renders a TriangleDiagram with dataIndex set so the print
  // function can collect them in order via querySelectorAll("svg[data-q-index]").
  // Cell size is generous (240px) so the diagram renders at good fidelity
  // before serialisation to the print window.
  const renderQCell = (q: TriQuestion, globalIdx: number, bgOverride?: string) => (
    <div
      key={globalIdx}
      className="rounded-lg shadow flex flex-col items-center gap-1 p-3"
      style={{ backgroundColor: bgOverride ?? stepBg, minHeight: 220 }}
    >
      ...
      <div style={{ width: 220, height: 200, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        <TriangleDiagram
          q={q}
          showAnswer={showWSAnswers}
          small
          labelBg={bgOverride ?? stepBg}
          dataIndex={globalIdx}
        />
      </div>
      {showWSAnswers && (
        <span className="text-sm font-bold text-center" style={{ color: "#059669" }}>{q.answer}</span>
      )}
    </div>
  );
      ...
      <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
        <div className="flex justify-center items-center gap-6 mb-4 flex-wrap">
          <div className="flex items-center gap-3">
            <label className="text-base font-semibold text-gray-700">Questions per level:</label>
        ...
        <div className="flex justify-center items-center gap-4">
          ...
          {worksheet.length > 0 && (<>
            ...
            <button
              onClick={() => handlePrint(worksheet, isDiff, showWSAnswers, TOOL_CONFIG.tools[currentTool].name, worksheetContainerRef)}
  ...
  const renderWhiteboard = () => {
    ...
    const questionBox = (
      <div className="rounded-xl flex items-center justify-center flex-shrink-0 flex-col gap-4 p-6"
        ...
        {showWBAnswer && question && <span className="text-3xl font-bold" style={{ color: "#166534" }}>{question.answer}</span>}
  ...
  // ── Worksheet render ────────────────────────────────────────────────────────
  // The outer div gets worksheetContainerRef so handlePrint can query SVGs inside it.
      ...
      <div ref={worksheetContainerRef} className="rounded-xl shadow-2xl p-8" style={{ backgroundColor: qBg }}>
        ...
        <div className="grid grid-cols-3 gap-4">
          {["level1", "level2", "level3"].map((lv, li) => {
            ...
            // Global index: level1 first, then level2, then level3 (matches generateWorksheet order)
            ...
            return (
              <div key={lv} className={`${c.bg} border-2 ${c.border} rounded-xl p-4`}>
                ...
                <div className="space-y-3">
                  {lqs.map((q, idx) => renderQCell(q, offset + idx, c.fill))}
                </div>
              </div>
            );
          })}
